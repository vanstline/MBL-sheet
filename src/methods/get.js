...
function getRangetxt(sheetIndex, range, currentIndex) {
  ...
  if (sheetIndex != currentIndex) {
    ...
    sheettxt = Store.MBLsheetfile[getSheetIndex(sheetIndex)].name.replace(
      /'/g,
      "''"
    );
...
function getMBLsheet_select_save() {
  return Store.MBLsheet_select_save;
}

function getMBLsheet_scroll_status() {
  return Store.MBLsheet_scroll_status;
}

function get_MBLsheetfile(plugin) {
  // 获取图表数据
  if (plugin) {
    Store.MBLsheetfile.forEach((file) => {
      if (!!file.chart) {
