  ...
  if (sp == "*") {
    ...
    if (getObjType(fp) == "array" && getObjType(tp) == "array") {
      ...
      if (getObjType(fp[0]) == "array" && getObjType(tp[0]) == "array") {
          ...
          for (let m = 0; m < fp.length; m++) {
            ...
            for (let n = 0; n < fp[m].length; n++) {
              ...
              if (isRealNum(fp[m][n]) && isRealNum(tp[m][n])) {
                value = MBLsheet_calcADPMM(fp[m][n], sp, tp[m][n]); //parseFloat(fp[m][n]) * parseFloat(tp[m][n]);
              } else {
                value = error.v;
              }
            ...
            for (let n = 0; n < collen; n++) {
              ...
              for (let p = 0; p < fp[0].length; p++) {
                ...
                if (isRealNum(fp[m][p]) && isRealNum(tp[p][n])) {
                  value += MBLsheet_calcADPMM(fp[m][p], sp, tp[p][n]); //parseFloat(fp[m][p]) * parseFloat(tp[p][n]);
                } else {
                  value += error.v;
                }
              }
            ...
            for (let n = 0; n < collen; n++) {
              ...
              for (let p = 0; p < tp[0].length; p++) {
                ...
                if (isRealNum(tp[m][p]) && isRealNum(fp[p][n])) {
                  value += MBLsheet_calcADPMM(fp[p][n], sp, tp[m][p]); //parseFloat(tp[m][p]) * parseFloat(fp[p][n]);
                } else {
                  value += error.v;
                }
              }
          ...
          for (let m = 0; m < fp.length; m++) {
            ...
            for (let n = 0; n < fp[m].length; n++) {
              ...
              if (isRealNum(fp[m][n]) && isRealNum(tp[n])) {
                value = MBLsheet_calcADPMM(fp[m][n], sp, tp[n]); //parseFloat(fp[m][n]) * parseFloat(tp[n]);
              } else {
                value = error.v;
              }
          ...
          for (let m = 0; m < rowlen; m++) {
            ...
            for (let n = 0; n < collen; n++) {
              ...
              if (isRealNum(fp[m][0]) && isRealNum(tp[n])) {
                value = MBLsheet_calcADPMM(fp[m][0], sp, tp[n]); // parseFloat(fp[m][0]) * parseFloat(tp[n]);
              } else {
                value = error.v;
              }
          ...
          for (let m = 0; m < tp.length; m++) {
            ...
            for (let n = 0; n < tp[m].length; n++) {
              ...
              if (isRealNum(fp[n]) && isRealNum(tp[m][n])) {
                value = MBLsheet_calcADPMM(fp[n], sp, tp[m][n]); // parseFloat(fp[n]) * parseFloat(tp[m][n]);
              } else {
                value = error.v;
              }
          ...
          for (let m = 0; m < rowlen; m++) {
            ...
            for (let n = 0; n < collen; n++) {
              ...
              if (isRealNum(fp[n]) && isRealNum(tp[m][0])) {
                value = MBLsheet_calcADPMM(fp[n], sp, tp[m][0]); //parseFloat(fp[n]) * parseFloat(tp[m][0]);
              } else {
                value = error.v;
              }
        ...
        for (let n = 0; n < fp.length; n++) {
          ...
          if (isRealNum(fp[n]) && isRealNum(tp[n])) {
            value = MBLsheet_calcADPMM(fp[n], sp, tp[n]); // parseFloat(fp[n]) * parseFloat(tp[n]);
          } else {
            value = error.v;
          }
      ...
      if (getObjType(fp[0]) == "array") {
        for (let m = 0; m < fp.length; m++) {
          ...
          for (let n = 0; n < fp[m].length; n++) {
            ...
            if (isRealNum(fp[m][n]) && isRealNum(tp)) {
              value = MBLsheet_calcADPMM(fp[m][n], sp, tp); // parseFloat(fp[m][n]) * parseFloat(tp);
            } else {
              value = error.v;
            }
        ...
        for (let n = 0; n < fp.length; n++) {
          ...
          if (isRealNum(fp[n]) && isRealNum(tp)) {
            value = MBLsheet_calcADPMM(fp[n], sp, tp); // parseFloat(fp[n]) * parseFloat(tp);
          } else {
            value = error.v;
          }
      ...
      if (getObjType(tp[0]) == "array") {
        for (let m = 0; m < tp.length; m++) {
          ...
          for (let n = 0; n < tp[m].length; n++) {
            ...
            if (isRealNum(fp) && isRealNum(tp[m][n])) {
              value = MBLsheet_calcADPMM(fp, sp, tp[m][n]); // parseFloat(fp) * parseFloat(tp[m][n]);
            } else {
              value = error.v;
            }
        ...
        for (let n = 0; n < tp.length; n++) {
          ...
          if (isRealNum(fp) && isRealNum(tp[n])) {
            value = MBLsheet_calcADPMM(fp, sp, tp[n]); //parseFloat(fp) * parseFloat(tp[n]);
          } else {
            value = error.v;
          }
      ...
      if (isRealNum(fp) && isRealNum(tp)) {
        result = MBLsheet_calcADPMM(fp, sp, tp); //parseFloat(fp) * parseFloat(tp);
      } else {
        result = error.v;
      }
            ...
            for (let n = 0; n < collen; n++) {
              ...
              for (let p = 0; p < fp[0].length; p++) {
                ...
                if (isRealNum(fp[m][p]) && isRealNum(tp_inverse[p][n])) {
                  value += MBLsheet_calcADPMM(fp[m][p], "*", tp_inverse[p][n]); // parseFloat(fp[m][p]) * parseFloat(tp_inverse[p][n]);
                } else {
                  value += error.v;
                }
              }
    ...
    if (getObjType(fp) == "array" && getObjType(tp) == "array") {
          ...
          for (let n = 0; n < fp[m].length; n++) {
            ...
            if (isRealNum(fp[m][n]) && isRealNum(tp[m][n])) {
              if (sp == "%" && parseFloat(tp[m][n]) == 0) {
                ...
                value = MBLsheet_calcADPMM(fp[m][n], sp, tp[m][n]); // eval(parseFloat(fp[m][n]) + sp + parseFloat(tp[m][n]));
              }
            } else {
              value = error.v;
            }
          ...
          for (let n = 0; n < fp[m].length; n++) {
            ...
            if (isRealNum(fp[m][n]) && isRealNum(tp[n])) {
              if (sp == "%" && parseFloat(tp[n]) == 0) {
                ...
                value = MBLsheet_calcADPMM(fp[m][n], sp, tp[n]); //eval(parseFloat(fp[m][n]) + sp + parseFloat(tp[n]));
              }
            } else {
              value = error.v;
            }
          ...
          for (let n = 0; n < tp[m].length; n++) {
            ...
            if (isRealNum(fp[n]) && isRealNum(tp[m][n])) {
              if (sp == "%" && parseFloat(tp[m][n]) == 0) {
                ...
                value = MBLsheet_calcADPMM(fp[n], sp, tp[m][n]); //eval(parseFloat(fp[n]) + sp + parseFloat(tp[m][n]));
              }
            } else {
              value = error.v;
            }
        ...
        for (let n = 0; n < fp.length; n++) {
          ...
          if (isRealNum(fp[n]) && isRealNum(tp[n])) {
            if (sp == "%" && parseFloat(tp[n]) == 0) {
              ...
              value = MBLsheet_calcADPMM(fp[n], sp, tp[n]); //eval(parseFloat(fp[n]) + sp + "(" + parseFloat(tp[n]) + ")" );
            }
          } else {
            value = error.v;
          }
          ...
          for (let n = 0; n < fp[m].length; n++) {
            ...
            if (isRealNum(fp[m][n]) && isRealNum(tp)) {
              if (sp == "%" && parseFloat(tp) == 0) {
                ...
                value = MBLsheet_calcADPMM(fp[m][n], sp, tp); //eval(parseFloat(fp[m][n]) + sp + parseFloat(tp));
              }
            } else {
              value = error.v;
            }
        ...
        for (let n = 0; n < fp.length; n++) {
          ...
          if (isRealNum(fp[n]) && isRealNum(tp)) {
            if (sp == "%" && parseFloat(tp) == 0) {
              ...
              value = MBLsheet_calcADPMM(fp[n], sp, tp); //eval(parseFloat(fp[n]) + sp + parseFloat(tp));
            }
          } else {
            value = error.v;
          }
          ...
          for (let n = 0; n < tp[m].length; n++) {
            ...
            if (isRealNum(fp) && isRealNum(tp[m][n])) {
              if (sp == "%" && parseFloat(tp[m][n]) == 0) {
                ...
                value = MBLsheet_calcADPMM(fp, sp, tp[m][n]); //eval(parseFloat(fp) + sp + parseFloat(tp[m][n]));
              }
            } else {
              value = error.v;
            }
        ...
        for (let n = 0; n < tp.length; n++) {
          ...
          if (isRealNum(fp) && isRealNum(tp[n])) {
            if (sp == "%" && parseFloat(tp[n]) == 0) {
              ...
              value = MBLsheet_calcADPMM(fp, sp, tp[n]); //eval(parseFloat(fp) + sp + parseFloat(tp[n]));
            }
          } else {
            value = error.v;
          }
      ...
      if (isRealNum(fp) && isRealNum(tp)) {
        if (sp == "%" && parseFloat(tp) == 0) {
          ...
          result = MBLsheet_calcADPMM(fp, sp, tp); //eval(parseFloat(fp) + sp + "(" + parseFloat(tp) + ")");
        }
      } else {
        result = error.v;
      }
...
function MBLsheet_getarraydata() {
  let fp = arguments[0];
...
function MBLsheet_calcADPMM(fp, sp, tp) {
  let value;
  if (sp == "+") {
...
function MBLsheet_getcelldata(txt) {
  if (window.MBLsheet_getcelldata_cache == null) {
    window.MBLsheet_getcelldata_cache = {};
  }

  if (txt in window.MBLsheet_getcelldata_cache) {
    return window.MBLsheet_getcelldata_cache[txt];
  }
  ...
  if (val.length > 1) {
    ...
    sheetdata = MBLsheetfile[index].data;
    rangetxt = val[0];
  ...
  if (rangetxt.indexOf(":") == -1) {
    ...
    if (!isNaN(row) && !isNaN(col)) {
      ...
      if (formula.execFunctionGlobalData != null) {
        let ef =
          formula.execFunctionGlobalData[row + "_" + col + "_" + sheetIndex];
        if (ef != null) {
          ret = ef;
        }
      }
      ...
      window.MBLsheet_getcelldata_cache[txt] = retAll;
    ...
    if (formula.execFunctionGlobalData != null) {
      for (let r = row[0]; r <= row[1]; r++) {
        for (let c = col[0]; c <= col[1]; c++) {
          let ef =
            formula.execFunctionGlobalData[r + "_" + c + "_" + sheetIndex];
          if (ef != null) {
...
function MBLsheet_parseData(value) {
...
function MBLsheet_indirect_check() {
...
function MBLsheet_indirect_check_return(txt) {
  return txt;
}

function MBLsheet_offset_check() {
  if (
...
function MBLsheet_getSpecialReference(isCellFirst, param1, param2) {
  ...
  if (isCellFirst) {
    return MBLsheet_getcelldata(rangeTxt + ":" + functionRange.startCell);
  } else {
    let rangeT = rangeTxt,
      sheetName = "";
    if (rangeTxt.indexOf("!") > -1) {
