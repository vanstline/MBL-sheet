...
export default function MBLsheetcreatedom(
  colwidth,
  rowheight,
  data,
  menu,
  ...
  let addControll =
    '<button id="MBLsheet-bottom-add-row" class="btn btn-default">' +
    locale_info.add +
    '</button><input id="MBLsheet-bottom-add-row-input" type="text" class="MBLsheet-datavisual-config-input MBLsheet-mousedown-cancel" placeholder="' +
    (MBLsheetConfigsetting.addRowCount || 100) +
    ...
    ")</span>";
  let backControll =
    ' <button id="MBLsheet-bottom-bottom-top" class="btn btn-default" style="">' +
    locale_info.backTop +
    "</button>";
  // let pageControll = ' <span id="MBLsheet-bottom-page-info" style="font-size: 14px;color: #f34141;">共'+ MBLsheetConfigsetting.pageInfo.totalPage +'页，当前已显示'+ (MBLsheetConfigsetting.pageInfo.currentPage) +'页，每页'+ MBLsheetConfigsetting.pageInfo.pageSize +'条</span> <button id="MBLsheet-bottom-page-next" class="btn btn-danger" style="">下一页</button>';
  let pageInfo = replaceHtml(locale_info.pageInfo, {
    total: MBLsheetConfigsetting.total ? MBLsheetConfigsetting.total : "",
  ...
  let pageControll =
    ' <span id="MBLsheet-bottom-page-info" style="font-size: 14px;color: #f34141;">' +
    pageInfo +
    '</span> <button id="MBLsheet-bottom-page-next" class="btn btn-danger" style="">下一页</button>';
  let pageControll2 =
    ' <span id="MBLsheet-bottom-page-info" style="font-size: 14px;color: #f34141;">' +
    pageInfo +
    "</span>";
  ...
  let bottomControll = "";
  if (MBLsheetConfigsetting.enableAddRow) {
    bottomControll += addControll;
  }

  if (MBLsheetConfigsetting.enablePage) {
    if (parseInt(MBLsheetConfigsetting.pageInfo.totalPage) == 1) {
  ...
  if (MBLsheetConfigsetting.enableAddBackTop) {
    bottomControll += backControll;
  }

  let flowstr = replaceHtml(
    '<div id="MBLsheetcoltable_0" class="MBLsheet-cell-flow-col"> <div id ="MBLsheet-sheettable_0" class="MBLsheet-cell-sheettable" style="height:${height}px;width:${width}px;"></div><div id="MBLsheet-bottom-controll-row" class="MBLsheet-bottom-controll-row"> ' +
      bottomControll +
      " </div> </div>",
  ...
  gh = replaceHtml(gh, {
    ...
    rowHeader:
      "<div style='height:" +
      Store.rh_height +
      "px' id='MBLsheetrowHeader_0' class='MBLsheetsheetchange'></div>",
    columnHeader: colsheader,
    functionButton: MBLsheetConfigsetting.functionButton,
  }); //设置需要显示的菜单
  ...
  $("#MBLsheet_info_detail_input").val(MBLsheetConfigsetting.title);
}
