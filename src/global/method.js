...
import { MBLsheetlodingHTML, luckyColor } from "../controllers/constant";
...
import { MBLsheetextendData } from "./extend";
import MBLsheetConfigsetting from "../controllers/MBLsheetConfigsetting";
import editor from "./editor";
import MBLsheetcreatesheet from "./createsheet";
...
const method = {
  ...
  addDataAjax: function (param, index, url, func) {
    ...
    $.ajax({
      ...
      success: function (d) {
        ...
        MBLsheetextendData(dataset["row"], newData);
  ...
  reload: function (param, index, url, func) {
    ...
    $.post(url, param, function (d) {
      ...
      MBLsheetcreatesheet(data[0].length, data.length, data, null, false);
      file["load"] = "1";
  ...
  clearSheetByIndex: function (i) {
    ...
    let sheetfile = Store.MBLsheetfile[index];
    ...
    if (!sheetfile.isPivotTable) {
      ...
      $("#" + Store.container + " .MBLsheet-data-visualization-chart").remove();
      $("#" + Store.container + " .MBLsheet-datavisual-selection-set").remove();

      $(
        "#MBLsheet-row-count-show, #MBLsheet-formula-functionrange-select, #MBLsheet-row-count-show, #MBLsheet-column-count-show, #MBLsheet-change-size-line, #MBLsheet-cell-selected-focus, #MBLsheet-selection-copy, #MBLsheet-cell-selected-extend, #MBLsheet-cell-selected-move, #MBLsheet-cell-selected"
      ).hide();

      delete sheetfile.load;
    } else {
      delete Store.MBLsheetfile[index];
    }
  },
  clear: function (index) {
    ...
    if (index == "all") {
      for (let i = 0; i < Store.MBLsheetfile.length; i++) {
        let sheetfile = Store.MBLsheetfile[i];
    ...
    sheetmanage.changeSheet(Store.MBLsheetfile[0].index);
  },
  destroy: function () {
    $("#" + Store.container).empty();
    $("body > .MBLsheet-cols-menu").remove();

    $(
      "#MBLsheet-modal-dialog-mask, #MBLsheetTextSizeTest, #MBLsheet-icon-morebtn-div"
    ).remove();
    $("#MBLsheet-input-box").parent().remove();
    $("#MBLsheet-formula-help-c").remove();
    $(".chartSetting, .MBLsheet-modal-dialog-slider").remove();

    //document event release
    $(document).off(".MBLsheetEvent");
    $(document).off(".MBLsheetProtection");

    //参数重置
    MBLsheetFreezen.initialHorizontal = true;
    MBLsheetFreezen.initialVertical = true;
    ...
    for (let key in defaultFormula) {
      if (key in MBLsheetformula) {
        MBLsheetformula[key] = defaultFormula[key];
      }
    }
  ...
  editorChart: function (c) {
    ...
    let chart_id = "MBLsheetEditMode-datav-chart";
    ...
    $("#" + chart_id)
      .find(".MBLsheet-modal-controll-update")
      .click();
  },
  /**
   * 获取单元格的值
  ...
  createHookFunction: function () {
    ...
    if (
      ...
      MBLsheetConfigsetting.hook[hookName] != null &&
      typeof MBLsheetConfigsetting.hook[hookName] == "function"
    ) {
      var args = Array.prototype.slice.apply(arguments);
      args.shift();
      let ret = MBLsheetConfigsetting.hook[hookName].apply(this, args);
      if (ret === false) {
        return false;
      } else {
        return true;
