...
const func_methods = {
  ...
  getFirstValue: function (data, nullCellType) {
    ...
    if (getObjType(data) == "array") {
      ...
      if (data.data == null) {
        ...
        let cell_r = window.MBLsheetCurrentRow;
        let cell_c = window.MBLsheetCurrentColumn;
  ...
  getCellBoolen: function (data) {
    ...
    if (getObjType(cumulative) == "boolean") {
      ...
      getObjType(cumulative) == "string" &&
      (cumulative.toLowerCase() == "true" ||
        cumulative.toLowerCase() == "false")
    ) {
  ...
  getCellDate: function (data) {
    ...
    if (getObjType(data) == "array") {
      ...
      if (
        data.data == null ||
        getObjType(data.data) == "array" ||
        isRealNull(data.data.v)
      ) {
        return error.v;
      }
  ...
  feb29Between: function (date1, date2) {
    ...
    if (
      _this.isLeapYear(year1) &&
      dayjs(date1) < dayjs(mar1year1) &&
      dayjs(date2) >= dayjs(mar1year1)
    ) {
      return true;
    }
    ...
    return (
      _this.isLeapYear(year2) &&
      dayjs(date2) >= dayjs(mar1year2) &&
      dayjs(date1) < dayjs(mar1year2)
    );
  },
  //SQL 查询
        ...
        for (let j = 0; j < criterias.length; ++j) {
          ...
          for (let p = 1; p < criteria.length; ++p) {
            currentCriteriaResult =
              currentCriteriaResult ||
              new Function("return " + database[k][l] + criteria[p])(); // jshint ignore:line
          }
        }
