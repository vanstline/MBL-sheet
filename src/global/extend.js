...
function MBLsheetextendtable(type, index, value, direction, sheetIndex) {
  ...
  let file = Store.MBLsheetfile[curOrder];
  ...
  //条件格式配置变动
  let CFarr = file.MBLsheet_conditionformat_save;
  let newCFarr = [];
  ...
  //交替颜色配置变动
  let AFarr = file.MBLsheet_alternateformat_save;
  let newAFarr = [];
  ...
  if (MBLsheetFreezen.freezenhorizontaldata != null && type == "row") {
    let freezen_scrollTop = MBLsheetFreezen.freezenhorizontaldata[2];
    let freezen_row_st = MBLsheetFreezen.freezenhorizontaldata[1] - 1;
    ...
    newFreezen.freezenhorizontaldata = [
      ...
      freezen_row_st + 1,
      freezen_scrollTop,
      MBLsheetFreezen.cutVolumn(Store.visibledatarow, freezen_row_st + 1),
      freezen_top,
    ];
  } else {
    newFreezen.freezenhorizontaldata = MBLsheetFreezen.freezenhorizontaldata;
  }

  if (MBLsheetFreezen.freezenverticaldata != null && type == "column") {
    let freezen_scrollLeft = MBLsheetFreezen.freezenverticaldata[2];
    let freezen_col_st = MBLsheetFreezen.freezenverticaldata[1] - 1;
    ...
    newFreezen.freezenverticaldata = MBLsheetFreezen.freezenverticaldata;
  }
  ...
  if (file.index == Store.currentSheetIndex) {
    ...
    file.MBLsheet_conditionformat_save = newCFarr;
...
function MBLsheetextendData(rowlen, newData) {
  ...
  Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)].data = d;
...
function MBLsheetdeletetable(type, st, ed, sheetIndex) {
  ...
  if (MBLsheetFreezen.freezenhorizontaldata != null && type == "row") {
    let freezen_scrollTop = MBLsheetFreezen.freezenhorizontaldata[2];
    let freezen_st = MBLsheet_searcharray(
...
//删除单元格
function MBLsheetDeleteCell(type, str, edr, stc, edc, sheetIndex) {
  sheetIndex = sheetIndex || Store.currentSheetIndex;
