...
export function getdatabyselection(range, sheetIndex) {
  if (range == null) {
    range = Store.MBLsheet_select_save[0];
  }
  ...
  if (sheetIndex != null && sheetIndex != Store.currentSheetIndex) {
    d = Store.MBLsheetfile[getSheetIndex(sheetIndex)]["data"];
    cfg = Store.MBLsheetfile[getSheetIndex(sheetIndex)]["config"];
  } else {
...
export function checkstatusByCell(cell, a) {
  ...
  if (a in tf || (a == "fs" && isInlineStringCell(cell))) {
    if (foucsStatus == null) {
      ...
      //     if (parseInt($("#MBLsheet-input-box").css("top")) > 0 && startContainer.parentNode.tagName=="SPAN" && !range.collapsed) {
