...
export function countfunc() {
  ...
  for (let s = 0; s < Store.MBLsheet_select_save.length; s++) {
    let data = getdatabyselectionNoCopy(Store.MBLsheet_select_save[s]);
  ...
  if (isFinite(max) || isFinite(min)) {
    ...
    ret +=
      "<span>" +
      locale_formula.average +
      ":" +
      update("w", Math.round((sum / count) * 10000) / 10000) +
      "</span>";
  }
