...
function MBLsheetRangeLast(obj) {
  let range;
...
function hideMenuByCancel(event) {
  // Right-click the menu in the title bar, and click on the elements whose class is MBLsheet-cols-rows-shift-left and MBLsheet-cols-rows-shift-right will trigger the hiding of the menu bar. It should be prohibited. Exclude these two elements. There may be more Other elements will also jump here for more testing

  if (
    event.target.classList &&
    (event.target.classList.contains("MBLsheet-cols-rows-shift-left") ||
      event.target.classList.contains("MBLsheet-cols-rows-shift-right"))
  ) {
    return;
  }
