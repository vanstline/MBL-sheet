...
import { MBLsheetMoveEndCell } from "./sheetMove";
...
function labelFilterOptionState(
  ...
  if (optionstate) {
    $top
      .addClass("MBLsheet-filter-options-active")
      .data("rowhidden", JSON.stringify(rowhidden))
      .data("caljs", JSON.stringify(caljs))
      .html(
        '<i class="fa fa-filter MBLsheet-mousedown-cancel" aria-hidden="true"></i>'
      );
    ...
    $top
      .removeClass("MBLsheet-filter-options-active")
      .data("rowhidden", "")
      .data("caljs", "")
      .html(
        '<i class="fa fa-caret-down MBLsheet-mousedown-cancel" aria-hidden="true"></i>'
      );
...
function createFilter() {
  ...
  if (Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)].isPivotTable) {
    return;
  }
  ...
  if (
    ...
    Store.MBLsheet_select_save = [{ row: [curR, curR], column: [st_c, ed_c] }];
    selectHightlightShow();

    Store.MBLsheet_shiftpositon = $.extend(true, {}, last);
    MBLsheetMoveEndCell("down", "range");
  } else if (last["row"][1] - last["row"][0] < 2) {
    Store.MBLsheet_shiftpositon = $.extend(true, {}, last);
    MBLsheetMoveEndCell("down", "range");
  }

  Store.MBLsheet_filter_save = $.extend(
    true,
    {},
    Store.MBLsheet_select_save[0]
  );

  createFilterOptions(Store.MBLsheet_filter_save);

  server.saveParam("all", Store.currentSheetIndex, Store.MBLsheet_filter_save, {
    k: "filter_select",
  });
  ...
  if (Store.filterchage) {
    Store.jfredo.push({
      ...
      filter_save: Store.MBLsheet_filter_save,
    });
  }
}

//创建筛选配置
function createFilterOptions(MBLsheet_filter_save, filterObj) {
  $("#MBLsheet-filter-selected-sheet" + Store.currentSheetIndex).remove();
  $("#MBLsheet-filter-options-sheet" + Store.currentSheetIndex).remove();

  if (
    MBLsheet_filter_save == null ||
    JSON.stringify(MBLsheet_filter_save) == "{}"
  ) {
    return;
  }
  ...
  let newSelectedHTML =
    ...
    Store.currentSheetIndex +
    '" class="MBLsheet-cell-selected MBLsheet-filter-selected"  style="left:' +
    col_pre +
    "px;width:" +
    (col - col_pre - 1) +
    "px;top:" +
  ...
  $("#MBLsheet-cell-main").append(newSelectedHTML);
  ...
  $("#MBLsheet-cell-main").append(
    '<div id="MBLsheet-filter-options-sheet' +
      Store.currentSheetIndex +
      '" class="MBLsheet-filter-options-c">' +
      optionHTML +
      "</div>"
  );
  $("#MBLsheet-rightclick-menu").hide();
  ...
  if ($("#MBLsheet-cell-main").scrollTop() > MBLsheet_filter_save["top_move"]) {
    $("#MBLsheet-scrollbar-y").scrollTop(MBLsheet_filter_save["top_move"]);
  }

  let file = Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)];

  file.filter_select = MBLsheet_filter_save;
}
...
function initialFilterHandler() {
  ...
  $("#MBLsheetfilter").click(createFilter);

  //右键菜单 菜单项hover
  let submenuhide = null,
    rightclickmenu = null;
  $(".MBLsheet-cols-menu .MBLsheet-cols-submenu").hover(
    function () {
      let $t = $(this),
        attrid = $t.attr("id"),
        $attr = $("#" + attrid + "_sub"),
  ...
  $(".MBLsheet-rightgclick-menu-sub").hover(
    function () {
      rightclickmenu.addClass("MBLsheet-cols-menuitem-hover");
      clearTimeout(submenuhide);
    },
    function () {
      rightclickmenu.removeClass("MBLsheet-cols-menuitem-hover");
      $(this).hide();
    }
  );

  $("#MBLsheet-filter-menu").mouseover(function () {
    clearTimeout(hidefilersubmenu);

    hidefilersubmenu = setTimeout(function () {
      $("#MBLsheet-filter-submenu").hide();
    }, 500);
  });

  $("#MBLsheet-filter-submenu")
    .mouseover(function () {
      clearTimeout(hidefilersubmenu);
    })
    .find(".MBLsheet-cols-menuitem")
    .click(function (e) {
      $("#MBLsheet-filter-selected span")
        .html($(this).find(".MBLsheet-cols-menuitem-content").text())
        .data("value", $(this).data("value"));
      $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input").hide();

      let $type = $(this).data("type");
      let $value = $(this).attr("data-value");

      if ($type == "2") {
        $("#MBLsheet-filter-selected span").data("type", "2");
        $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input2").show();
        $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input input").prop(
          "type",
          "number"
        );
      } else if ($type == "0") {
        $("#MBLsheet-filter-selected span").data("type", "0");
      } else {
        $("#MBLsheet-filter-selected span").data("type", "1");
        $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input").eq(0).show();
      ...
      $("#MBLsheet-filter-byvalue").next().slideUp();
      $("#MBLsheet-filter-submenu").hide();
    });

  $("#MBLsheet-filter-bycondition, #MBLsheet-filter-byvalue").click(
    function () {
      let $t = $(this);
      ...
      setTimeout(function () {
        if (
          $t.attr("id") == "MBLsheet-filter-bycondition" &&
          $("#MBLsheet-filter-bycondition").next().is(":visible")
        ) {
          if (
            $("#MBLsheet-filter-selected span").text() !=
            locale_filter.filiterInputNone
          ) {
            $("#MBLsheet-filter-byvalue").next().slideUp(200);
          }
        }

        if ($t.is($("#MBLsheet-filter-bycondition"))) {
          if (
            $("#MBLsheet-filter-bycondition").next().is(":hidden") &&
            $("#MBLsheet-filter-byvalue").next().is(":hidden")
          ) {
            $("#MBLsheet-filter-byvalue").next().slideDown(200);
          }
        }
      }, 300);
    }
  ...
  $("#MBLsheet-filter-selected").click(function () {
    let $t = $(this),
      toffset = $t.offset(),
      $menu = $("#MBLsheet-filter-submenu");
    $menu.hide();
  ...
  $("#MBLsheet-cell-main").on(
    ...
    function (e) {
      ...
      $("#MBLsheet-filter-menu, #MBLsheet-filter-submenu").hide();
      $("#MBLsheet-filter-byvalue-input").val("");
      $("#MBLsheet-filter-bycondition").next().hide();
      $("#MBLsheet-filter-byvalue").next().show();
      ...
      $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input")
        .hide()
        .find("input")
        .val();
      $("#MBLsheet-filter-selected span")
      ...
      if (byconditiontype == "2") {
        let $input = $("#MBLsheet-filter-menu .MBLsheet-filter-selected-input2")
          .show()
          .find("input");
      ...
      $("#MBLsheet-filter-orderby-asc")
        .off("click")
        .on("click", function () {
          orderbydatafiler(st_r, st_c, ed_r, ed_c, cindex, true);
        });

      $("#MBLsheet-filter-orderby-desc")
        .off("click")
        .on("click", function () {
          orderbydatafiler(st_r, st_c, ed_r, ed_c, cindex, false);
        });

      const loadingObj = MBLsheetlodingHTML("#MBLsheet-filter-byvalue-select", {
        text: locale_filter.filiterMoreDataTip,
      });
      $("#MBLsheet-filter-byvalue-select").empty().append(loadingObj.el);
      ...
      setTimeout(function () {
        ...
        //$("#MBLsheet-filter-byvalue-select").html("<div class='ListBox MBLsheet-mousedown-cancel' style='min-height: 100px; max-height: " + containerH + "px; overflow-y: auto; overflow-x: hidden;'><table cellspacing='0' style='width:100%;' class='MBLsheet-mousedown-cancel'>" + item.join("") + "</table></div>");

        $("#MBLsheet-filter-byvalue-select").append(
          "<div class='ListBox MBLsheet-mousedown-cancel' style='min-height: 100px; max-height: " +
            containerH +
            "px; overflow-y: auto; overflow-x: hidden;'><table cellspacing='0' style='width:100%;' class='MBLsheet-mousedown-cancel'>" +
            item.join("") +
            "</table></div>"
        );
        loadingObj.close();
  ...
  //按颜色筛选
  $("#MBLsheet-filter-orderby-color").hover(
    function () {
      //遍历筛选列颜色
      let $menu = $("#MBLsheet-filter-menu");
      ...
      if (JSON.stringify(bgMap).length > 2 && Object.keys(bgMap).length > 1) {
        ...
        for (let b in bgMap) {
          if (bgMap[b] == 0) {
            bgColorItemHtml +=
              '<div class="item MBLsheet-mousedown-cancel"><label class="MBLsheet-mousedown-cancel" style="background-color: ' +
              b +
              '" title="' +
              b +
              '"></label><input class="MBLsheet-mousedown-cancel" type="checkbox" checked="checked"/></div>';
          } else {
            bgColorItemHtml +=
              '<div class="item MBLsheet-mousedown-cancel"><label class="MBLsheet-mousedown-cancel" style="background-color: ' +
              b +
              '" title="' +
              b +
              '"></label><input class="MBLsheet-mousedown-cancel" type="checkbox"/></div>';
          }
        }
        filterBgColorHtml =
          '<div id="filterBgColor" class="box MBLsheet-mousedown-cancel"><div class="title MBLsheet-mousedown-cancel">' +
          locale_filter.filiterByColorTip +
          '</div><div style="max-height:128px;overflow:auto;" class="MBLsheet-mousedown-cancel">' +
          bgColorItemHtml +
          "</div></div>";
      }
      ...
      if (JSON.stringify(fcMap).length > 2 && Object.keys(fcMap).length > 1) {
        ...
        filterFcColorHtml =
          '<div id="filterFcColor" class="box MBLsheet-mousedown-cancel"><div class="title MBLsheet-mousedown-cancel">' +
      ...
      if (filterBgColorHtml == "" && filterFcColorHtml == "") {
        content =
          '<div class="MBLsheet-mousedown-cancel" style="padding: 10px 30px;text-align: center;">' +
        ...
        content =
          filterBgColorHtml +
          filterFcColorHtml +
          '<div class="MBLsheet-mousedown-cancel"><button id="MBLsheet-filter-orderby-color-confirm" class="btn btn-primary MBLsheet-mousedown-cancel" style="margin: 5px 20px;width: 70px;">' +
          locale_button.confirm +
          "</button></div>";
      }
      //颜色筛选子菜单
      $("#MBLsheet-filter-orderby-color-submenu").remove();
      $("body").append(
        '<div id="MBLsheet-filter-orderby-color-submenu" class="MBLsheet-cols-menu MBLsheet-mousedown-cancel">' +
          content +
          "</div>"
      );
      let $t = $("#MBLsheet-filter-orderby-color-submenu").end();
      let $con = $(this).parent();
      let winW = $(window).width(),
        winH = $(window).height();
      let menuW = $con.width(),
      ...
      $("#MBLsheet-filter-orderby-color-submenu")
        .css({ top: top, left: left })
        .show();
    },
    function () {
      submenuhide = setTimeout(function () {
        $("#MBLsheet-filter-orderby-color-submenu").hide();
      }, 200);
    }
  );
  ...
  $(document).on(
    "click",
    "#MBLsheet-filter-orderby-color-submenu .item label",
    function () {
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-orderby-color-submenu #MBLsheet-filter-orderby-color-confirm",
      function () {
        let bg_colorMap = {};
        let fc_colorMap = {};

        $("#MBLsheet-filter-orderby-color-submenu .item").each(function (i, e) {
        ...
        let bg_filter;
        if (
          $("#MBLsheet-filter-orderby-color-submenu #filterBgColor").length > 0
        ) {
          bg_filter = true;
        } else {
          bg_filter = false;
        ...
        let fc_filter;
        if (
          $("#MBLsheet-filter-orderby-color-submenu #filterFcColor").length > 0
        ) {
          fc_filter = true;
        } else {
          fc_filter = false;
        ...
        $(
          "#MBLsheet-filter-menu, #MBLsheet-filter-submenu, #MBLsheet-filter-orderby-color-submenu"
        ).hide();
        cleargridelement();
      }
    );
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-byvalue-select .textBox",
      function () {
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-byvalue-select .year",
      function () {
        if ($(this).attr("data-check") == "true") {
          $(this).attr("data-check", "false");
          $(this)
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-byvalue-select .month",
      function () {
        //月份 对应的 天
        if ($(this).attr("data-check") == "true") {
          $(this).attr("data-check", "false");
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-byvalue-select .day",
      function () {
        if ($(this).attr("data-check") == "true") {
          $(this).attr("data-check", "false");
          $(this).find("input[type='checkbox']").removeAttr("checked");
  ...
  $(document)
    ...
    .on(
      ...
      "#MBLsheet-filter-byvalue-select .yearBox .fa-caret-right",
      function (event) {
  ...
  //全选
  $("#MBLsheet-filter-byvalue-btn-all").click(function () {
    $("#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']").prop(
      "checked",
      true
    );
    $("#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']")
      .parents(".MBLsheet-mousedown-cancel")
      .attr("data-check", "true");
  });

  //清除
  $("#MBLsheet-filter-byvalue-btn-clear").click(function () {
    $(
      "#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']"
    ).removeAttr("checked");
    $("#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']")
  ...
  //反选
  $("#MBLsheet-filter-byvalue-btn-contra").click(function () {
    let $input = $(
      "#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']"
    );
    $input.each(function (i, e) {
      if ($(e).is(":checked")) {
        ...
        $(e).parents(".MBLsheet-mousedown-cancel").attr("data-check", "false");
      } else {
        $(e).prop("checked", true);
        $(e).parents(".MBLsheet-mousedown-cancel").attr("data-check", "true");
      }
    });
    //天 对应的 月份
    let $month = $("#MBLsheet-filter-byvalue-select .ListBox .monthBox");
    ...
    //天 对应的 年份
    let $year = $("#MBLsheet-filter-byvalue-select .ListBox .yearBox");
  ...
  //清除筛选
  $("#MBLsheet-filter-initial").click(function () {
    if (!checkProtectionAuthorityNormal(Store.currentSheetIndex, "filter")) {
      return;
    }
    ...
    redo["filter_save"] = $.extend(true, {}, Store.MBLsheet_filter_save);
    ...
    $(
      ...
      let optionstate = $t.hasClass("MBLsheet-filter-options-active");
    ...
    //清除筛选发送给后台
    Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)].filter = null;
  ...
  //按照值进行筛选
  $("#MBLsheet-filter-byvalue-input").on("input propertychange", function () {
    let v = $(this).val().toString();
    $(
      "#MBLsheet-filter-byvalue-select .ListBox .MBLsheet-mousedown-cancel"
    ).show();

    if (v != "") {
      $("#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']").each(
        function (i, e) {
  ...
  //筛选取消
  $("#MBLsheet-filter-cancel").click(function () {
    $("#MBLsheet-filter-menu, #MBLsheet-filter-submenu").hide();
  });

  //筛选 确认
  $("#MBLsheet-filter-confirm").click(function () {
    let $menu = $("#MBLsheet-filter-menu");
    let st_r = $menu.data("str"),
      ed_r = $menu.data("edr"),
      cindex = $menu.data("cindex"),
    ...
    if (
      $("#MBLsheet-filter-bycondition").next().is(":visible") &&
      $("#MBLsheet-filter-byvalue").next().is(":hidden") &&
      $("#MBLsheet-filter-selected span").data("value") != "null"
    ) {
      let $t = $("#MBLsheet-filter-selected span");
      ...
      if (type == "0") {
        ...
        let $input = $(
          "#MBLsheet-filter-menu .MBLsheet-filter-selected-input2 input"
        );
        caljs["type"] = "2";
    ...
    let optionstate =
      $(
        "#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']:visible:checked"
      ).length <
        $(
          "#MBLsheet-filter-byvalue-select .ListBox input[type='checkbox']:visible"
        ).length ||
      $("#MBLsheet-filter-byvalue-input").val().length > 0 ||
      ($("#MBLsheet-filter-bycondition").next().is(":visible") &&
        $("#MBLsheet-filter-byvalue").next().is(":hidden") &&
        $("#MBLsheet-filter-selected span").data("value") != "null");
