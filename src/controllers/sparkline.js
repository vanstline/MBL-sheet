...
const MBLsheetSparkline = {
  ...
  bar: {
    ...
    renderRegion: function (valuenum, highlight) {
      ...
      for (i = 0; i < valcount; i++) {
        ...
        result.push(
          target.drawRect(y, x, height - 1, this.barWidth - 1, color, color)
        );
      }
      if (result.length === 1) {
        return result[0];
  ...
  _drawShape: function (shapeid, path, lineColor, fillColor, lineWidth) {
    ...
    context.beginPath();
    context.moveTo(
      path[0][0] + 0.5 + this.offsetX,
      path[0][1] + 0.5 + this.offsetY
    );

    for (i = 1, plen = path.length; i < plen; i++) {
      context.lineTo(
        path[i][0] + 0.5 + this.offsetX,
        path[i][1] + 0.5 + this.offsetY
  ...
  _canvasID: "MBLsheetTableContent",
  render: function (
    shapeseq,
    shapes,
    offsetX,
...
let _MBLsheetSparkLineOptions = {
  get: function (type) {
    return MBLsheetSparkline.mergedOptions[type];
  },
};

let _MBLsheetSparkLineTarget = {
  drawLine: function (x1, y1, x2, y2, lineColor, lineWidth) {
    return MBLsheetSparkline.drawLine(x1, y1, x2, y2, lineColor, lineWidth);
  },

  drawShape: function (path, lineColor, fillColor, lineWidth) {
    return MBLsheetSparkline.drawShape(path, lineColor, fillColor, lineWidth);
  },

  drawCircle: function (x, y, radius, lineColor, fillColor, lineWidth) {
    return MBLsheetSparkline.drawCircle(
      x,
      y,
      radius,
      lineColor,
  ...
  drawPieSlice: function (
    ...
    return MBLsheetSparkline.drawPieSlice(
      x,
      y,
      radius,
      startAngle,
  ...
  drawRect: function (x, y, width, height, lineColor, fillColor) {
    return MBLsheetSparkline.drawRect(
      x,
      y,
      width,
      height,
...
for (let item in MBLsheetSparkline) {
  if (
    item in
    {
      line: null,
    ...
    MBLsheetSparkline[item].options = _MBLsheetSparkLineOptions;
    MBLsheetSparkline[item].target = _MBLsheetSparkLineTarget;
  }

  if (item in { bar: null, column: null, tristate: null, discrete: null }) {
    MBLsheetSparkline[item].changeHighlight = barHighlightMixin.changeHighlight;
    MBLsheetSparkline[item].render = barHighlightMixin.render;
  }
}

export default MBLsheetSparkline;
