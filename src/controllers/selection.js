...
import { replaceHtml, getObjType, MBLsheetfontformat } from "../utils/util";
...
const selection = {
  clearcopy: function (e) {
    ...
    if (!clipboardData) {
      let textarea = $("#MBLsheet-copy-content").css("visibility", "hidden");
      textarea.val(cpdata);
      textarea.focus();
      textarea.select();
  ...
  copy: function (e) {
    ...
    for (let s = 0; s < Store.MBLsheet_select_save.length; s++) {
      let range = Store.MBLsheet_select_save[s];
      ...
      Store.MBLsheet_selection_range.push({
        row: range.row,
        column: range.column,
      });
  ...
  paste: function (e, triggerType) {
    ...
    setTimeout(function () {
      ...
      if (
        data.indexOf("MBLsheet_copy_action_table") > -1 &&
        ...
        if (Store.MBLsheet_paste_iscut) {
          ...
          _this.pasteHandlerOfCutPaste(Store.MBLsheet_copy_save);
          _this.clearcopy(e);
        } else {
          _this.pasteHandlerOfCopyPaste(Store.MBLsheet_copy_save);
        }
      } else if (data.indexOf("MBLsheet_copy_action_image") > -1) {
  ...
  pasteHandlerOfCutPaste: function (copyRange) {
    ...
    let c_dataVerification = $.extend(
      true,
      {},
      Store.MBLsheetfile[getSheetIndex(copySheetIndex)]["dataVerification"]
    );
    ...
    if (Store.currentSheetIndex != copySheetIndex) {
      ...
      let sourceData = $.extend(
        true,
        [],
        Store.MBLsheetfile[getSheetIndex(copySheetIndex)]["data"]
      );
      let sourceConfig = $.extend(
        true,
        {},
        Store.MBLsheetfile[getSheetIndex(copySheetIndex)]["config"]
      );
      ...
      let source_cdformat = $.extend(
        true,
        [],
        Store.MBLsheetfile[getSheetIndex(copySheetIndex)][
  ...
  pasteHandlerOfCopyPaste: function (copyRange) {
    ...
    let cdformat = null;
    if (copyRange["copyRange"].length == 1) {
      let c_file = Store.MBLsheetfile[getSheetIndex(copySheetIndex)];
      let a_file = Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)];

      let ruleArr_cf = $.extend(
        true,
        [],
        c_file["MBLsheet_conditionformat_save"]
      );

      if (ruleArr_cf != null && ruleArr_cf.length > 0) {
        cdformat = $.extend(true, [], a_file["MBLsheet_conditionformat_save"]);
