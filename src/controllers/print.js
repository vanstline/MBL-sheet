...
export function viewChange(curType, preType) {
  ...
  server.saveParam("cg", Store.currentSheetIndex, curType, {
    k: "curentsheetView",
  });
...
function switchViewBtn($t) {
  let $viewList = $t.parent(),
    preType = $viewList.find("MBLsheet-print-viewBtn-active").attr("type");
  ...
  $t.parent()
    .find(".MBLsheet-print-viewBtn")
    .removeClass("MBLsheet-print-viewBtn-active");
  $t.addClass("MBLsheet-print-viewBtn-active");
}

export function printInitial() {
  let container = MBLsheetConfigsetting.container;
  let _this = this;
  $("#" + container)
    .find(".MBLsheet-print-viewBtn")
    .click(function () {
