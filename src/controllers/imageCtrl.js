...
const imageCtrl = {
  ...
  modelHtml: function (id, imgItem) {
    ...
    return `<div id="${id}" class="MBLsheet-modal-dialog MBLsheet-modal-dialog-image" style="width:${width}px;height:${height}px;padding:0;position:${position};left:${left}px;top:${top}px;z-index:200;">
                    <div class="MBLsheet-modal-dialog-content" style="width:100%;height:100%;overflow:hidden;position:relative;">
      ...
      -imgItem.crop.offsetLeft * Store.zoomRatio
                    ...
                    <div class="MBLsheet-modal-dialog-border" style="border:${borderWidth}px ${
  ...
  getSliderHtml: function () {
    ...
    return `<div id="MBLsheet-modal-dialog-slider-imageCtrl" class="MBLsheet-modal-dialog-slider MBLsheet-modal-dialog-slider-imageCtrl" style="display:block;">
                    <div class="MBLsheet-modal-dialog-slider-title">
                        <span>${imageText.imageSetting}</span>
                        <span class="MBLsheet-model-close-btn" title="${imageText.close}">
  ...
  sliderHtmlShow: function () {
    ...
    $("#MBLsheet-modal-dialog-slider-imageCtrl").remove();
    ...
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemType" + type).prop(
      "checked",
      true
    );
    ...
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemIsFixedPos").prop(
      "checked",
      isFixedPos
    );
    ...
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemBorderWidth").val(
      border.width
    );
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemBorderRadius").val(
      border.radius
    );
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemBorderStyle").val(
      border.style
    );
    $("#MBLsheet-modal-dialog-slider-imageCtrl #imgItemBorderColor span")
  ...
  colorSelectDialog: function (currenColor) {
    ...
    $("#MBLsheet-imageCtrl-colorSelect-dialog").remove();

    $("body").append(
      replaceHtml(modelHTML, {
        id: "MBLsheet-imageCtrl-colorSelect-dialog",
        addclass: "MBLsheet-imageCtrl-colorSelect-dialog",
        ...
        botton: `<button id="MBLsheet-imageCtrl-colorSelect-dialog-confirm" class="btn btn-primary">${locale_button.confirm}</button>
                        <button class="btn btn-default MBLsheet-model-close-btn">${locale_button.cancel}</button>`,
        style: "z-index:100003",
      })
    );
    let $t = $("#MBLsheet-imageCtrl-colorSelect-dialog")
    ...
    $("#MBLsheet-imageCtrl-colorSelect-dialog")
      .css({
    ...
    $("#MBLsheet-imageCtrl-colorSelect-dialog")
      ...
      .spectrum({
        ...
        move: function (color) {
          ...
          $("#MBLsheet-imageCtrl-colorSelect-dialog .currenColor span")
  ...
  init: function () {
    ...
    //关闭
    $(
      "#MBLsheet-modal-dialog-slider-imageCtrl .MBLsheet-model-close-btn"
    ).click(function () {
    ...
    //常规
    $("#MBLsheet-modal-dialog-slider-imageCtrl")
      .off("change.radio")
      .on(
        "change.radio",
    ...
    $(document)
      ...
      .on(
        ...
        "#MBLsheet-imageCtrl-colorSelect-dialog-confirm",
        function () {
    ...
    //image active
    $("#MBLsheet-image-showBoxs")
      .off("mousedown.active")
      .on("mousedown.active", ".MBLsheet-modal-dialog-image", function (e) {
        if (
        ...
        $("#MBLsheet-modal-dialog-activeImage").show().css({
          width: width,
          height: height,
          left: left,
          top: top,
        ...
        $(
          "#MBLsheet-modal-dialog-activeImage .MBLsheet-modal-dialog-border"
        ).css({
    ...
    //image move
    $("#MBLsheet-modal-dialog-activeImage")
      .off("mousedown.move")
      .on("mousedown.move", ".MBLsheet-modal-dialog-content", function (e) {
        if (
          !checkProtectionAuthorityNormal(
            Store.currentSheetIndex,
            "editObjects",
        ...
        if (!$("#MBLsheet-modal-dialog-slider-imageCtrl").is(":visible")) {
          _this.sliderHtmlShow();
        }
        ...
        let offset = $("#MBLsheet-modal-dialog-activeImage").offset();
    ...
    $("#MBLsheet-modal-dialog-activeImage")
      ...
      .on(
        "mousedown.resize",
        ".MBLsheet-modal-dialog-resize-item",
        function (e) {
          if (
            !checkProtectionAuthorityNormal(
              Store.currentSheetIndex,
          ...
          let position = $("#MBLsheet-modal-dialog-activeImage").position();
          let width = $("#MBLsheet-modal-dialog-activeImage").width();
          let height = $("#MBLsheet-modal-dialog-activeImage").height();
    ...
    //image croppingExit
    $("#MBLsheet-modal-dialog-cropping")
    ...
    $("#MBLsheet-image-showBoxs")
      ...
      .on(
        "mousedown.restore",
        ".MBLsheet-modal-controll-restore",
        function (e) {
    ...
    $("#MBLsheet-image-showBoxs")
      ...
      .on("mousedown.delete", ".MBLsheet-modal-controll-del", function (e) {
        _this.removeImgItem();
        e.stopPropagation();
      });
  },
  ...
  cancelActiveImgItem: function () {
    ...
    $("#" + _this.currentImgId + " .MBLsheet-modal-dialog-border").css({
  ...
  addImgItem: function (img) {
    ...
    $("#MBLsheet-image-showBoxs .img-list").append(modelHtml);
  ...
  moveImgItem: function () {
    ...
    let obj = $("#MBLsheet-modal-dialog-activeImage")[0];
  ...
  croppingEnter: function () {
    ...
    $("#MBLsheet-modal-dialog-cropping .MBLsheet-modal-dialog-border").css({
  ...
  croppingExit: function () {
    ...
    $("#MBLsheet-modal-dialog-activeImage .MBLsheet-modal-dialog-content").css({
      "background-image": "url(" + imgSrc + ")",
      "background-size":
        item.default.width + "px " + item.default.height + "px",
      "background-position":
  ...
  copyImgItem: function (e) {
    ...
    let cpdata =
      '<table data-type="MBLsheet_copy_action_image"><tr><td><td></tr></table>';

    if (!clipboardData) {
      let textarea = $("#MBLsheet-copy-content");
  ...
  allImagesShow: function () {
    ...
    $("#MBLsheet-modal-dialog-cropping").hide();
    $("#MBLsheet-modal-dialog-slider-imageCtrl").hide();
    $("#MBLsheet-image-showBoxs .img-list").empty();
