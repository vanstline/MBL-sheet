...
function initialCellFormatModelEvent() {
  ...
  $("#MBLsheet-cellFormat-confirm").click(function () {
    let locked = $("#MBLsheet-protection-check-locked").is(":checked");
    let hidden = $("#MBLsheet-protection-check-hidden").is(":checked");
    ...
    $("#MBLsheet-cellFormat-config").hide();
  ...
  if (
    ...
    if (data != null) {
      for (let i = 0; i < Store.MBLsheet_select_save.length; i++) {
        let selection = Store.MBLsheet_select_save[i];
...
function initialCellFormatModel() {
  ...
  $("body").append(
    replaceHtml(modelHTML, {
      id: "MBLsheet-cellFormat-config",
      addclass: "MBLsheet-cellFormat-config",
      title: local_cellFormat.cellFormatTitle,
      content: `
            <div class="MBLsheet-cellFormat-menu-c">
                <div class="MBLsheet-cellFormat-menu MBLsheet-cellFormat-menu-active" id="MBLsheet-cellFormat-protection">
                    ${local_cellFormat.protection}
                </div>
            </div>
            <div id="MBLsheet-cellFormat-protection-content" class="MBLsheet-cellFormat-content">
                <div class="MBLsheet-cellFormat-protection">
                    <p>
                        ${local_cellFormat.protectionTips}
                    </p>
                    <label for="MBLsheet-protection-check-locked"><input id="MBLsheet-protection-check-locked" name="MBLsheet-protection-check-locked" type="checkbox">${local_cellFormat.locked}</label><span>部分选中</span>
                    <br/>
                    <label for="MBLsheet-protection-check-hidden"><input id="MBLsheet-protection-check-hidden" name="MBLsheet-protection-check-hidden" type="checkbox">${local_cellFormat.hidden}</label><span>全部选中</span>
                </div>
            </div>
        `,
      botton: `<button id="MBLsheet-cellFormat-confirm" class="btn btn-primary">${locale_button.confirm}</button>
...
export function openCellFormatModel() {
  ...
  $("#MBLsheet-protection-check-locked")
    .prop("checked", locked)
    .parent()
    .next()
    .html(tipsLock);
  $("#MBLsheet-protection-check-hidden")
    .prop("checked", hidden)
    .parent()
    .next()
    .html(tipshidden);

  openSelfModel("MBLsheet-cellFormat-config");
}
