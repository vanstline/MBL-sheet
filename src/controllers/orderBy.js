...
export function orderByInitial() {
  ...
  //菜单栏 排序按钮
  $("#MBLsheetorderbyasc, #MBLsheetorderbyasc_t").mousedown(function (event) {
    cleargridelement(event);
    sortColumnSeletion(Store.orderbyindex, true);
    selectHightlightShow();
  });

  $("#MBLsheetorderbydesc, #MBLsheetorderbydesc_t").click(function (event) {
    cleargridelement(event);
  ...
  //排序事件
  let MBLsheet_sort_initial = true;
  $("#MBLsheetorderby").click(function () {
    ...
    if (MBLsheet_sort_initial) {
      ...
      $("#MBLsheet-sort-dialog .MBLsheet-sort-dialog-additem").click(
        function () {
          ...
          let option = "",
            i = $("#MBLsheet-sort-dialog table").data("itemcount") + 1;
          let t = $("#MBLsheet-sort-haveheader").is(":checked");
          ...
          $("#MBLsheet-sort-dialog table").append(`
                    <tr class="MBLsheet-sort-dialog-tr">
                        <td><span class="MBLsheet-sort-item-close" onclick="$(this).parent().parent().remove();"><i class="fa fa-times"
                ...
                `);
          $("#MBLsheet-sort-dialog table").data("itemcount", i);
        }
      );

      $("#MBLsheet-sort-haveheader").change(function () {
        let last = Store.MBLsheet_select_save[0];
        let r1 = last["row"][0],
          r2 = last["row"][1];
        let c1 = last["column"][0],
        ...
        $("#MBLsheet-sort-dialog tr select").each(function () {
          $(this).html(option);
        });
      });

      //Custom sort
      $("#MBLsheet-sort-modal-confirm").click(function () {
        if (Store.MBLsheet_select_save.length > 1) {
          if (isEditMode()) {
            alert(locale_sort.noRangeError);
          } else {
        ...
        $($("#MBLsheet-sort-dialog table tr").toArray().reverse()).each(
          function () {
    ...
    $("#MBLsheet-sort-dialog select").html(option);

    $("#MBLsheet-sort-dialog .MBLsheet-sort-dialog-tr").remove();

    $("#MBLsheet-sort-haveheader").prop("checked", false);
    $("#MBLsheet-sort-dialog input:radio:first").prop("checked", "checked");

    $("#MBLsheet-sort-dialog .MBLsheet-modal-dialog-title-text").html(
    ...
    $("#MBLsheet-sort-dialog-tablec").css("max-height", (winh - myh) / 2);
    $("#MBLsheet-sort-dialog")
      .css({
    ...
    if (r1 < r2) {
      setTimeout(function () {
        ...
        if (hastitle) {
          $("#MBLsheet-sort-haveheader").prop("checked", true).change();
        }
      }, 10);
    }
  });
