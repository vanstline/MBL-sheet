...
export function zoomChange(ratio) {
  ...
  clearTimeout(MBLsheetZoomTimeout);
  MBLsheetZoomTimeout = setTimeout(() => {
    ...
    //批注
    MBLsheetPostil.buildAllPs(currentSheet.data);
    ...
    server.saveParam("all", Store.currentSheetIndex, Store.zoomRatio, {
      k: "zoomRatio",
    });
    server.saveParam(
      "cg",
...
export function zoomRefreshView() {
  // let $scrollLeft = $("#MBLsheet-scrollbar-x"), $scrollTop = $("#MBLsheet-scrollbar-y");
...
export function zoomInitial() {
  ...
  $("#MBLsheet-zoom-minus").click(function () {
    let currentRatio;
  ...
  $("#MBLsheet-zoom-plus").click(function () {
    let currentRatio;
    if (Store.zoomRatio == null) {
      currentRatio = Store.zoomRatio = 1;
    } else {
  ...
  $("#MBLsheet-zoom-slider").mousedown(function (e) {
  ...
  $("#MBLsheet-zoom-cursor")
    .mousedown(function (e) {
      let curentX = e.pageX,
        cursorLeft = parseFloat($("#MBLsheet-zoom-cursor").css("left"));
      $("#MBLsheet-zoom-cursor").css("transition", "none");
      $(document)
        ...
        .on("mousemove.zoomCursor", function (event) {
          ...
          // curentX = moveX;
          // let left = parseFloat($("#MBLsheet-zoom-cursor").css("left"));
          ...
          $("#MBLsheet-zoom-ratioText").html(r);
          $("#MBLsheet-zoom-cursor").css("left", pos - 4);
        });
      ...
      $(document)
        ...
        .on("mouseup.zoomCursor", function (event) {
          ...
          $("#MBLsheet-zoom-cursor").css("transition", "all 0.3s");
        });
  ...
  $("#MBLsheet-zoom-ratioText").click(function () {
    // Store.zoomRatio = 1;
    zoomChange(1);
    zoomNumberDomBind(1);
  });
...
function zoomSlierDomBind(ratio) {
  ...
  $("#MBLsheet-zoom-cursor").css("left", domPos - 4);
}
