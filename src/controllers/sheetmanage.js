...
import MBLsheetcreatedom from "../global/createdom";
import tooltip from "../global/tooltip";
import formula from "../global/formula";
import { MBLsheetrefreshgrid, jfrefreshgrid_rhcw } from "../global/refresh";
import rhchInit from "../global/rhchInit";
import editor from "../global/editor";
import { MBLsheetextendtable, MBLsheetdeletetable } from "../global/extend";
...
import { sheetHTML, MBLsheetlodingHTML } from "./constant";
...
const sheetmanage = {
  ...
  getCurSheetnoset: function () {
    ...
    for (let i = 0; i < Store.MBLsheetfile.length; i++) {
      if (Store.MBLsheetfile[i].status == 1) {
        curindex = Store.MBLsheetfile[i].index;
        break;
      }
    }
  ...
  getCurSheet: function () {
    if (Store.MBLsheetfile.length) {
      ...
      if (!hasActive) {
        Store.MBLsheetfile[0].status = 1;
      }
    }
    Store.currentSheetIndex = Store.MBLsheetfile[0].index;

    for (let i = 0; i < Store.MBLsheetfile.length; i++) {
      if (Store.MBLsheetfile[i].status == 1) {
        Store.currentSheetIndex = Store.MBLsheetfile[i].index;
        break;
      }
    }
  ...
  addNewSheet: function (e, isPivotTable) {
    ...
    let order = Store.MBLsheetfile.length;
    ...
    Store.MBLsheetfile.push(sheetconfig);
  ...
  setSheetHide: function (index, isDelete) {
    ...
    if (
      ...
      !method.createHookFunction("sheetHideBefore", {
        sheet: Store.MBLsheetfile[currentIdx],
      })
    ) {
      return;
    }
    Store.MBLsheetfile[currentIdx].hide = 1;

    let MBLsheetcurrentSheetitem = $("#MBLsheet-sheets-item" + index);
    MBLsheetcurrentSheetitem.hide();
    ...
    let indicator;
    if (MBLsheetConfigsetting.showsheetbarConfig.sheet) {
      indicator = MBLsheetcurrentSheetitem.nextAll(":visible");
      if (MBLsheetcurrentSheetitem.nextAll(":visible").length > 0) {
        indicator = indicator.eq(0).data("index");
      } else {
        indicator = MBLsheetcurrentSheetitem.prevAll(":visible")
          .eq(0)
          .data("index");
      }
    } else {
      let nextActiveIdx,
        showSheetIdxs = [];
      Store.MBLsheetfile.forEach((ele, index) => {
      ...
      indicator = Store.MBLsheetfile[nextActiveIdx].index;
    }

    $("#MBLsheet-sheets-item" + indicator).addClass(
      "MBLsheet-sheets-item-active"
    );
    ...
    server.saveParam("sh", MBLsheetcurrentSheetitem.data("index"), 1, {
      op: "hide",
      cur: indicator,
    });
    // 钩子 sheetHideAfter
  ...
  setSheetShow: function (index) {
    let _this = this;
    const file = Store.MBLsheetfile[_this.getSheetIndex(index)];
    // 钩子 sheetShowBefore
  ...
  getCurrentOrder: function () {
    ...
    $("#MBLsheet-sheet-area div.MBLsheet-sheets-item").each(function (a) {
      let index = $(this).data("index");

      for (let i = 0; i < Store.MBLsheetfile.length; i++) {
        if (Store.MBLsheetfile[i].index == index) {
  ...
  createSheet: function () {
    ...
    let btn = [];
    Store.MBLsheetfile.sort(_this.ordersheet("order"));

    for (let i = 0; i < Store.MBLsheetfile.length; i++) {
      let display = "";
      let sheetIndex = Store.MBLsheetfile[i].index;

      let colorset = "";
      if (Store.MBLsheetfile[i].color != null) {
        colorset =
          '<div class="MBLsheet-sheets-item-color" style=" position: absolute; width: 100%; height: 3px; bottom: 0px; left: 0px; background-color: ' +
          Store.MBLsheetfile[i].color +
          ';"></div>';
      }

      if (Store.currentSheetIndex == sheetIndex) {
        //使用Store.MBLsheetfile中的index比较，而不是order
        btn.push(
          replaceHtml(sheetHTML, {
            index: sheetIndex,
            active: "MBLsheet-sheets-item-active",
            name: Store.MBLsheetfile[i].name,
            style: "",
            colorset: colorset,
          })
        );
      } else {
        if (Store.MBLsheetfile[i].hide == 1) {
          btn.push(
            replaceHtml(sheetHTML, {
              index: sheetIndex,
              active: "",
      ...
      //Store.MBLsheetfile[i].index = i; //index即为默认
      ...
      $("#MBLsheet-cell-main").append(
        "<div " +
          display +
          ' id="MBLsheet-datavisual-selection-set-' +
          sheetIndex +
          '" class="MBLsheet-datavisual-selection-set"></div>'
      );
    }

    $("#MBLsheet-sheet-container-c").append(btn.join(""));

    _this.locationSheet();
  },
  // *控制sheet栏的左右滚动按钮是否显示
  locationSheet: function () {
    let $c = $("#MBLsheet-sheet-container-c"),
      winW = $("#" + Store.container).width();
    let $cursheet = $(
      "#MBLsheet-sheet-container-c > div.MBLsheet-sheets-item-active"
    ).eq(0);

    let scrollLeftpx = 0;
    let c_width = 0;

    $("#MBLsheet-sheet-container-c > div.MBLsheet-sheets-item:visible").each(
      function () {
        if ($(this).hasClass("MBLsheet-sheets-item-active")) {
    ...
    setTimeout(function () {
      ...
      if (MBLsheetConfigsetting.showsheetbarConfig.sheet) {
        if (c_width >= winW * 0.7) {
          $("#MBLsheet-sheet-area .MBLsheet-sheets-scroll").css(
            "display",
            "inline-block"
          );
          $("#MBLsheet-sheet-container .docs-sheet-fade-left").show();
        } else {
          $("#MBLsheet-sheet-area .MBLsheet-sheets-scroll").css(
            "display",
            "none"
          );
          $("#MBLsheet-sheet-container .docs-sheet-fade-left").hide();
        }
      }
    }, 1);
  },
  copySheet: function (copyindex, e) {
    ...
    let copyjson = $.extend(true, {}, Store.MBLsheetfile[copyarrindex]);
    ...
    if (
      !method.createHookFunction("sheetCopyBefore", {
        targetSheet: Store.MBLsheetfile[copyarrindex],
        copySheet: copyjson,
      })
    ) {
      return;
    ...
    let copyobject = $("#MBLsheet-sheets-item" + copyindex);
    $("#MBLsheet-sheet-container-c").append(
      replaceHtml(sheetHTML, {
    ...
    $("#MBLsheet-sheets-item" + copyjson.index).insertAfter(copyobject);
    Store.MBLsheetfile.splice(copyarrindex + 1, 0, copyjson);
  ...
  createSheetbydata: function (data, isrenew, isBefore = true) {
    ...
    if (isBefore) {
      ...
      if (previndex >= Store.MBLsheetfile.length) {
        previndex = Store.MBLsheetfile.length - 1;
        $("#MBLsheet-sheets-item" + data.index).insertAfter(
          $("#MBLsheet-sheets-item" + Store.MBLsheetfile[previndex].index)
        );
      } else {
        $("#MBLsheet-sheets-item" + data.index).insertBefore(
          $("#MBLsheet-sheets-item" + Store.MBLsheetfile[previndex].index)
        );
      }
    }

    Store.MBLsheetfile.push(data);

    $("#MBLsheet-sheet-area div.MBLsheet-sheets-item").removeClass(
      "MBLsheet-sheets-item-active"
    );
    $("#MBLsheet-sheets-item" + data.index).addClass(
      "MBLsheet-sheets-item-active"
    );
    $("#MBLsheet-cell-main").append(
      '<div id="MBLsheet-datavisual-selection-set-' +
  ...
  deleteSheet: function (index) {
    ...
    const file = Store.MBLsheetfile[arrIndex];
    ...
    $("#MBLsheet-sheets-item" + index).remove();
    $("#MBLsheet-datavisual-selection-set-" + index).remove();

    let removedsheet = Store.MBLsheetfile.splice(arrIndex, 1);
    _this.reOrderAllSheet();
  ...
  buildGridData: function (file) {
    ...
    //亿万格式+精确度 恢复全局初始化
    MBLsheetConfigsetting.autoFormatw = false;
    MBLsheetConfigsetting.accuracy = undefined;
    return data;
  },
  ...
  sheetParamRestore: function (file, data) {
    Store.MBLsheet_select_save = file["MBLsheet_select_save"];
    if (
    ...
    Store.MBLsheet_selection_range =
      file["MBLsheet_selection_range"] == null
        ? []
        : file["MBLsheet_selection_range"];
    ...
    if (file["defaultRowHeight"] != null) {
      ...
      Store.defaultrowlen = MBLsheetConfigsetting["defaultRowHeight"];
    }

    if (file["defaultColWidth"] != null) {
      Store.defaultcollen = parseFloat(file["defaultColWidth"]);
    } else {
      Store.defaultcollen = MBLsheetConfigsetting["defaultColWidth"];
    }
  ...
  initialjfFile: function (menu, title) {
    ...
    _this.getCurSheet();
    let file = Store.MBLsheetfile[_this.getSheetIndex(Store.currentSheetIndex)];
    ...
    file.MBLsheet_select_save = select_save;

    this.sheetParamRestore(file, data);

    let r2 = Store.MBLsheet_select_save[0].row[1],
    ...
    if (Store.MBLsheet_select_save.length > 1) {
      for (let i = 0; i < Store.MBLsheet_select_save.length; i++) {
        if (Store.MBLsheet_select_save[i].row[1] > r2) {
          r2 = Store.MBLsheet_select_save[i].row[1];
        }

        if (Store.MBLsheet_select_save[i].column[1] > c2) {
    ...
    //钩子函数 表格创建之前触发
    if (typeof MBLsheetConfigsetting.beforeCreateDom == "function") {
      MBLsheetConfigsetting.beforeCreateDom(MBLsheet);
    }

    if (typeof MBLsheetConfigsetting.workbookCreateBefore == "function") {
      MBLsheetConfigsetting.workbookCreateBefore(MBLsheet);
    }

    // Store.flowdata = data;

    MBLsheetcreatedom(colwidth, rowheight, data, menu, title);

    setTimeout(function () {
      tooltip.createHoverTip(
        "#MBLsheet_info_detail",
        ".MBLsheet_info_detail_back, .MBLsheet_info_detail_input, .MBLsheet_info_detail_update"
      );
      tooltip.createHoverTip(
        "#MBLsheet-wa-editor",
      ...
      Store.MBLsheetTableContentHW = [
        $("#MBLsheet-cell-main").width() +
          Store.rowHeaderWidth -
          Store.cellMainSrollBarSize,
        $("#MBLsheet-cell-main").height() +
      ...
      let ini = function () {
        ...
        let execF = function () {
          ...
          formula.execFunctionGroupForce(
            MBLsheetConfigsetting.forceCalculation
          );
          _this.restoreSheetAll(Store.currentSheetIndex);

          // MBLsheetrefreshgrid(0, 0);
          $("#MBLsheet_info_detail_save").html(locale_info.detailSave);
          ...
          if (file["scrollLeft"] != null && file["scrollLeft"] > 0) {
            $("#MBLsheet-scrollbar-x").scrollLeft(file["scrollLeft"]);
          } else {
            $("#MBLsheet-scrollbar-x").scrollLeft(0);
          }

          if (file["scrollTop"] != null && file["scrollTop"] > 0) {
            $("#MBLsheet-scrollbar-y").scrollTop(file["scrollTop"]);
          } else {
          ...
          if (MBLsheetConfigsetting.pointEdit) {
            setTimeout(function () {
        ...
        if (loadSheetUrl == "") {
          ...
          // for(let i = 0;i<Store.MBLsheetfile.length;i++){
          //     let otherfile = Store.MBLsheetfile[i];
          //     if(otherfile.index == file.index){
          //         continue;
          //     }
          //     // let otherfile = Store.MBLsheetfile[_this.getSheetIndex(item)];
  ...
  storeSheetParam: function () {
    ...
    file["MBLsheet_selection_range"] = $.extend(
      true,
      [],
      Store.MBLsheet_selection_range
    );
    ...
    if (
      $("#MBLsheet-scrollbar-x")[0].scrollWidth >
      $("#MBLsheet-scrollbar-x")[0].offsetWidth
    ) {
      file["scrollLeft"] = $("#MBLsheet-scrollbar-x").scrollLeft(); //横向滚动条
    }

    if (
      $("#MBLsheet-scrollbar-y")[0].scrollHeight >
      $("#MBLsheet-scrollbar-y")[0].offsetHeight
    ) {
      file["scrollTop"] = $("#MBLsheet-scrollbar-y").scrollTop(); //纵向滚动条
    }
  ...
  setSheetParam: function (isload = true) {
    ...
    //批注
    MBLsheetPostil.buildAllPs(Store.flowdata);
  ...
  restoreselect: function () {
    ...
    if (file["scrollLeft"] != null && file["scrollLeft"] > 0) {
      $("#MBLsheet-scrollbar-x").scrollLeft(file["scrollLeft"]); //列标题
    } else {
      $("#MBLsheet-scrollbar-x").scrollLeft(0);
    }

    if (file["scrollTop"] != null && file["scrollTop"] > 0) {
      $("#MBLsheet-scrollbar-y").scrollTop(file["scrollTop"]); //列标题
    } else {
      $("#MBLsheet-scrollbar-y").scrollTop(0);
    }
  },
  storeSheetParamALL: function () {
    ...
    Store.MBLsheetfile[index]["data"] = Store.flowdata;
    Store.MBLsheetfile[index]["config"] = $.extend(true, {}, Store.config);
  },
  mergeCalculationSheet: {},
  mergeCalculation: function (index) {
    let file = Store.MBLsheetfile[this.getSheetIndex(index)];
  ...
  changeSheet: function (index, isPivotInitial, isNewSheet, isCopySheet) {
    ...
    if (server.allowUpdate) {
      $("#MBLsheet-cell-main #MBLsheet-multipleRange-show").empty();
      server.multipleIndex = 0;
    }
    let file = Store.MBLsheetfile[_this.getSheetIndex(index)];
    // 钩子 sheetCreateAfter
    if (isNewSheet) {
    ...
    $(
      "#MBLsheet-filter-selected-sheet" +
    ...
    if (load != null) {
      ...
      if (
        ...
        $("#MBLsheet-grid-window-1").append(MBLsheetlodingHTML());
    ...
    $("#MBLsheet-cell-main .MBLsheet-datavisual-selection-set").hide();
    $("#MBLsheet-datavisual-selection-set-" + index).show();

    //隐藏其他sheet的图表，显示当前sheet的图表 chartMix
    renderChartShow(index);

    MBLsheetFreezen.initialFreezen(index);
    _this.restoreselect();
  },
  ...
  checkLoadSheetIndex: function (file) {
    ...
    if (calchain != null) {
      ...
      for (let i = 0; i < calchain.length; i++) {
        ...
        if (formulaTxt == null) {
          let file = Store.MBLsheetfile[this.getSheetIndex(dataindex)];
  ...
  showSheet: function () {
    ...
    $("#MBLsheet-cell-flow_0").css({ width: Store.ch_width, top: "-1px" }); //width更新
    $("#MBLsheet-sheettable_0").css({
      width: Store.ch_width - 1,
      height: Store.rh_height,
    });
    $("#MBLsheetrowHeader_0").css("height", Store.rh_height);
    $("#MBLsheet-cols-h-cells_0").css("width", Store.ch_width); //width更新

    $("#MBLsheet-scrollbar-x div").width(Store.ch_width);
    $("#MBLsheet-scrollbar-y div").height(
      Store.rh_height +
        Store.columnHeaderHeight -
        Store.cellMainSrollBarSize -
        3
    ...
    if (file["scrollTop"] != null && file["scrollTop"] > 0) {
      $("#MBLsheet-scrollbar-y").scrollTop(file["scrollTop"] * Store.zoomRatio);
    } else {
      $("#MBLsheet-scrollbar-y").scrollTop(0);
    }
  ...
  setCurSheet: function (index) {
    for (let i = 0; i < Store.MBLsheetfile.length; i++) {
      if (Store.MBLsheetfile[i]["index"] == index) {
        Store.MBLsheetfile[i].status = 1;
      } else {
        Store.MBLsheetfile[i].status = 0;
      }
    }
  ...
  changeSheetExec: function (index, isPivotInitial, isNewSheet, isCopySheet) {
    let $sheet = $("#MBLsheet-sheets-item" + index);

    window.MBLsheet_getcelldata_cache = null;
    $("#MBLsheet-sheet-area div.MBLsheet-sheets-item").removeClass(
      "MBLsheet-sheets-item-active"
    );
    $sheet.addClass("MBLsheet-sheets-item-active").show();
  ...
  sheetArrowShowAndHide() {
    const $wrap = $("#MBLsheet-sheet-container-c");
    ...
    if (sw > w) {
      ...
      $("#MBLsheet-sheet-area .MBLsheet-sheets-scroll").css("display", "none");
      $("#MBLsheet-sheet-container .docs-sheet-fade-left").hide();
    }
  },
  // *显示sheet栏左右的灰色
  sheetBarShowAndHide(index) {
    ...
    if (c_width + scrollLeft >= c_srollwidth) {
      $("#MBLsheet-sheet-container .docs-sheet-fade-right").hide();
    } else {
      $("#MBLsheet-sheet-container .docs-sheet-fade-right").show();
    }
  },
    ...
    if (file.chart == null) {
      ...
      for (let i = 0; i < file.chart.length; i++) {
        if (file.chart[i].chart_id == chart_id) {
          Store.MBLsheetfile[index].chart.splice(i, 1);
          break;
        }
      }
    }
  ...
  saveChart: function (json) {
    //采用chartMix store存储，弃用Store.MBLsheetfile存储，防止重复存储
  ...
  getRangetxt: function (sheetIndex, range, currentIndex) {
    ...
    if (sheetIndex != currentIndex) {
      sheettxt = Store.MBLsheetfile[this.getSheetIndex(sheetIndex)].name + "!";
    }
  ...
  getSheetName: function (sheetIndex) {
    ...
    return Store.MBLsheetfile[this.getSheetIndex(sheetIndex)].name;
  },
  ...
  getSheetData: function (sheetIndex) {
    ...
    return Store.MBLsheetfile[this.getSheetIndex(sheetIndex)].data;
  },
  getSheetConfig: function (sheetIndex) {
    ...
    let config = Store.MBLsheetfile[_this.getSheetIndex(sheetIndex)].config;

    if (config == null) {
      Store.MBLsheetfile[_this.getSheetIndex(sheetIndex)].config = {};
    }

    return Store.MBLsheetfile[_this.getSheetIndex(sheetIndex)].config;
  },
  restoreFilter: function (sheetIndex) {
    let index = this.getSheetIndex(sheetIndex);
    let file = Store.MBLsheetfile[index];

    // if($('#MBLsheet-filter-selected-sheet' + sheetIndex).length > 0 || file.filter_select == null || JSON.stringify(file.filter_select) == "{}"){
  ...
  restoreFreezen: function (sheetIndex) {
    MBLsheetFreezen.initialFreezen(sheetIndex);
  },
  ...
  execCache: function (item) {
    ...
    if (type == "sha") {
      Store.MBLsheetfile.push(value);
    } else if (type == "shc") {
      let copyjson = $.extend(
        true,
        {},
        Store.MBLsheetfile[_this.getSheetIndex(value.copyindex)]
      );
      copyjson.index = index;
      Store.MBLsheetfile.push(copyjson);
    } else if (type == "shd") {
      Store.MBLsheetfile.splice(value.deleIndex, 1);
    } else if (type == "shr") {
      for (let pos in value) {
        Store.MBLsheetfile[_this.getSheetIndex(pos)].order = value[pos];
      }
    }
    ...
    if (type == "v") {
      ...
      if (op == "hide") {
        file.status = 0;
        Store.MBLsheetfile[_this.getSheetIndex(cur)].status = 1;
      } else if (op == "show") {
        for (let i = 0; i < Store.MBLsheetfile.length; i++) {
          Store.MBLsheetfile[i].status = 0;
        }
      ...
      MBLsheetdeletetable(mtype, mst, med, true);
    } else if (type == "arc") {
      let rc = item.rc,
        index = value.index,
        len = value.len;
      ...
      MBLsheetextendtable(mtype, index, len, true);
    } else if (type == "na") {
