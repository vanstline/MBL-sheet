...
function MBLsheetMoveEndCell(postion, type, isScroll, terminal, onlyvalue) {
  if (isScroll == null) {
    isScroll = true;
  }
  ...
  let last = Store.MBLsheet_select_save[Store.MBLsheet_select_save.length - 1];
  ...
  if (type == "range") {
    // need var
    var p_startR = Store.MBLsheet_shiftpositon["row"][0];
    var p_startC = Store.MBLsheet_shiftpositon["column"][0];

    let p_endR = Store.MBLsheet_shiftpositon["row"][1];
    let p_endC = Store.MBLsheet_shiftpositon["column"][1];
  ...
  if (!onlyvalue) {
    if (type == "cell") {
      MBLsheetMoveHighlightCell(moveP, moveV, "rangeOfSelect", isScroll);
    } else if (type == "range") {
      MBLsheetMoveHighlightRange(moveP, moveV, "rangeOfSelect", isScroll);
    }
  } else {
    return moveV;
  }
...
//方向键  调整单元格
function MBLsheetMoveHighlightCell(postion, index, type, isScroll) {
  if (isScroll == null) {
    isScroll = true;
  }
...
//ctrl + 方向键  调整单元格
function MBLsheetMoveHighlightCell2(postion, type, isScroll) {
  if (!isScroll) {
    isScroll = true;
  }
  ...
  if (type == "rangeOfSelect") {
    ...
    Store.MBLsheet_select_save = [{ row: rowseleted, column: columnseleted }];
    selectHightlightShow();
...
//shift + 方向键  调整选区
function MBLsheetMoveHighlightRange(postion, index, type, isScroll) {
  if (isScroll == null) {
    isScroll = true;
  }
...
//ctrl + shift + 方向键  调整选区
function MBLsheetMoveHighlightRange2(postion, type, isScroll) {
  if (!isScroll) {
    isScroll = true;
  }
