...
export function updateInlineStringFormat(cell, attr, value, $input) {
  ...
  let $textEditor;
  if (cac.id == "MBLsheet-rich-text-editor") {
    $textEditor = $(cac);
  } else {
    $textEditor = $(cac).closest("#MBLsheet-rich-text-editor");
  }
  let $functionbox = $(cac).closest("#MBLsheet-functionbox-cell");

  if (
    $textEditor.length == 0 &&
    $functionbox.length == 0 &&
    ...
    $functionbox = $(cac).closest("#MBLsheet-functionbox-cell");
  }
  ...
  if ($textEditor.length > 0) {
    if (startContainer === endContainer) {
      ...
      if (left != "") {
        ...
        if (inherit) {
          let box = $(span).closest("#MBLsheet-input-box").get(0);
          if (box != null) {
      ...
      if (
        ...
        for (let i = 0; i < startSpanIndex; i++) {
          ...
          cont +=
            "<span style='" + span.style.cssText + "'>" + content + "</span>";
        }
        if (sleft != "") {
          cont +=
            "<span style='" +
        ...
        if (eright != "") {
          cont +=
            "<span style='" + endSpan.style.cssText + "'>" + eright + "</span>";
        }
        ...
        selectTextContentCross(
          spans.get(startSeletedNodeIndex),
          spans.get(endSeletedNodeIndex)
        );
      }
    }
...
export function enterKeyControll(cell) {
  ...
  if ($textEditor.length > 0) {
    let startSpan = startContainer.parentNode;
    if (startContainer.id == "MBLsheet-rich-text-editor") {
    ...
    selectTextContentCollapse(
      $textEditor.find("span").get(spanIndex),
      startOffset + 1
    );
  } else if ($functionbox.length > 0) {
  }
