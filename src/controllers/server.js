...
const server = {
  ...
  openWebSocket: function () {
    ...
    if ("WebSocket" in window) {
      ...
      _this.websocket.onmessage = function (result) {
        ...
        // 用户退出时，关闭协同编辑时其提示框
        if (message === "用户退出") {
          $("#MBLsheet-multipleRange-show-" + id).hide();
        ...
        if (type == 1) {
          ...
          const sheetToUpdate = Store.MBLsheetfile.filter(
          ...
          if (sheetToUpdate !== null) {
            setTimeout(() => {
              ...
              $(`#MBLsheet-sheets-item${oldIndex}`).attr("data-index", index);
              $(`#MBLsheet-sheets-item${oldIndex}`).prop(
                "id",
                `MBLsheet-sheets-item${index}`
              );
              $(`#MBLsheet-datavisual-selection-set-${oldIndex}`).prop(
                "id",
                `MBLsheet-datavisual-selection-set-${index}`
              );
            }, 1);
          }
        } else if (type == 2) {
          ...
          Store.cooperativeEdit.checkoutData.forEach((item) => {
            if (item.index != Store.currentSheetIndex) {
              $("#MBLsheet-multipleRange-show-" + item.id).hide();
              item.op == "";
            }
          });

          if ($("#MBLsheet-multipleRange-show-" + id)[0]) {
            let change_bottom =
              $("#MBLsheet-multipleRange-show-" + id)[0].offsetHeight - 1;
            $("#MBLsheet-multipleRange-show-" + id + ">.username").css({
    ...
    if (type == "v") {
      ...
      if (index == Store.currentSheetIndex) {
        ...
        if (value != null && value.ps != null) {
          MBLsheetPostil.buildPs(r, c, value.ps);
        } else {
          MBLsheetPostil.buildPs(r, c, null);
        }
        ...
        for (let r = r1; r <= r2; r++) {
          for (let c = c1; c <= c2; c++) {
            if (
              ...
              MBLsheetPostil.buildPs(r, c, value[r - r1][c - c1].ps);
            } else {
              MBLsheetPostil.buildPs(r, c, null);
            }
          }
      ...
      if (k == "name") {
        //工作表名
        $("#MBLsheet-sheet-container-c #MBLsheet-sheets-item" + index)
          .find("span.MBLsheet-sheets-item-name")
          .html(value);
      } else if (k == "color") {
        //工作表颜色
        ...
        currentSheetItem.find(".MBLsheet-sheets-item-color").remove();
        ...
        //PivotTable
        // MBLsheet.pivotTable.changePivotTable(index);
      } else if (k == "frozen") {
        //freezen row and column

        // tranform frozen
        MBLsheetFreezen.frozenTofreezen();

        if (index == Store.currentSheetIndex) {
          const _locale = locale();
          const locale_freezen = _locale.freezen;
          if (file["freezen"].horizontal == null) {
            $("#MBLsheet-freezen-btn-horizontal").html(
      ...
      } else if (k == "MBLsheet_conditionformat_save") {
        //条件格式
        if (index == Store.currentSheetIndex) {
          setTimeout(function () {
            MBLsheetrefreshgrid();
      ...
      } else if (k == "MBLsheet_alternateformat_save") {
        //交替颜色
        if (index == Store.currentSheetIndex) {
          setTimeout(function () {
            MBLsheetrefreshgrid();
      ...
      for (let i = 0; i < Store.MBLsheetfile.length; i++) {
        if (Store.MBLsheetfile[i].index == value.deleIndex) {
          // 如果删除的是当前sheet，则切换到前一个sheet页
          if (Store.currentSheetIndex === value.deleIndex) {
            const index = value.deleIndex;

            Store.MBLsheetfile[sheetmanage.getSheetIndex(index)].hide = 1;
            ...
            let indicator = MBLsheetcurrentSheetitem.nextAll(":visible");
          ...
          server.sheetDeleSave.push(Store.MBLsheetfile[i]);

          Store.MBLsheetfile.splice(i, 1);

          break;
        }
      }

      $("#MBLsheet-sheets-item" + value.deleIndex).remove();
      $("#MBLsheet-datavisual-selection-set-" + value.deleIndex).remove();
      sheetmanage.locationSheet();
    } else if (type == "shr") {
      //sheet位置
      for (let x in value) {
        Store.MBLsheetfile[getSheetIndex(x)].order = value[x];
      }
    } else if (type == "shre") {
      //删除sheet恢复操作
      ...
      if (op == "hide") {
        file.hide = 1;
        $("#MBLsheet-sheets-item" + index).hide();

        if (index == Store.currentSheetIndex) {
          $("#MBLsheet-sheets-item" + cur).addClass(
            "MBLsheet-sheets-item-active"
          );
        ...
        file.hide = 0;
        $("#MBLsheet-sheets-item" + index).show();
      }
      sheetmanage.locationSheet();
    } else if (type == "c") {
      //图表操作 TODO
  ...
  multipleRangeShow: function (id, name, r, c, value) {
    ...
    if ($("#MBLsheet-multipleRange-show-" + id).length > 0) {
      $("#MBLsheet-multipleRange-show-" + id).css({
        position: "absolute",
        left: col_pre - 1,
        width: col - col_pre - 1,
        top: row_pre - 1,
      ...
      $("#MBLsheet-multipleRange-show-" + id + " .username").text(name);
      $("#MBLsheet-multipleRange-show-" + id + " .username").show();
      ...
      // let itemHtml = '<div id="MBLsheet-multipleRange-show-'+ id +'" data-color="'+ luckyColor[_this.multipleIndex] +'" title="'+ name +'" style="position: absolute;left: '+ (col_pre - 1) +'px;width: '+ (col - col_pre - 1) +'px;top: '+ (row_pre - 1) +'px;height: '+ (row - row_pre - 1) +'px;border: 1px solid '+ luckyColor[_this.multipleIndex] +';z-index: 15;">'+
      ...
      let itemHtml = `<div
								id="MBLsheet-multipleRange-show-${id}"
								class="MBLsheet-multipleRange-show"
      ...
      $(itemHtml).appendTo(
        $("#MBLsheet-cell-main #MBLsheet-multipleRange-show")
      );
      ...
      if (_this.updateUrl != "") {
        $.post(
          ...
          function (data) {
            ...
            if (re.status) {
              $("#MBLsheet_info_detail_update").html(
                "最近存档时间:" + dayjs().format("M-D H:m:s")
              );
              $("#MBLsheet_info_detail_save").html("同步成功");
              _this.clearcachelocaldata();
            } else {
              $("#MBLsheet_info_detail_save").html(
    ...
    html2canvas(
        ...
        .get(0),
      {
        onrendered: function (canvas) {
          //let imgcut = $("#MBLsheet-cell-main").find(".MBLsheet-grid-window");
          ...
          let curindex = MBLsheet.sheetmanage.getCurSheetnoset();
