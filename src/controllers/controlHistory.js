...
const controlHistory = {
  redo: function (e) {
    ...
    if (
      ...
      Store.currentSheetIndex != ctr.sheetIndex
    ) {
    ...
    if (ctr.type == "datachange") {
      ...
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].config = Store.config;

      if (ctr.ctrlType == "resizeR") {
        server.saveParam("cg", ctr.sheetIndex, ctr.config["rowlen"], {
          k: "rowlen",
        });
      } else if (ctr.ctrlType == "resizeC") {
        server.saveParam("cg", ctr.sheetIndex, ctr.config["columnlen"], {
          k: "columnlen",
        });
      }

      let images = $.extend(true, {}, ctr.images);
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].images = images;
      ...
      //config
      Store.config = ctr.config;
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].config = ctr.config;

      server.saveParam("cg", ctr.sheetIndex, ctr.config["rowhidden"], {
        k: "rowhidden",
      });
      ...
      server.saveParam("cg", ctr.sheetIndex, ctr.config["colhidden"], {
        k: "colhidden",
      });
      ...
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].pivotTable =
        ctr.pivotTable;
      ...
      for (let i = 0; i < Store.MBLsheetfile.length; i++) {
        if (Store.MBLsheetfile[i].name == ctr.name) {
          isDupName = true;
        }
      }
      ...
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].name = ctr.oldtxt;
      $("#MBLsheet-sheets-item" + ctr.sheetIndex)
        .find(".MBLsheet-sheets-item-name")
        .html(ctr.oldtxt);

      server.saveParam("all", ctr.sheetIndex, ctr.oldtxt, { k: "name" });
    } else if (ctr.type == "sheetColor") {
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].color = ctr.oldcolor;

      let MBLsheetcurrentSheetitem = $(
        "#MBLsheet-sheets-item" + ctr.sheetIndex
      );
      ...
      dataVerificationCtrl.ref(
        ctr.currentDataVerification,
        ctr.historyDataVerification,
        ctr.sheetIndex
      );
      ...
      hyperlinkCtrl.ref(
        ctr.currentHyperlink,
        ctr.historyHyperlink,
        ctr.sheetIndex,
        ctr.data,
        ctr.range
      );
      ...
      for (let i = 0; i < historyRules.length; i++) {
        ...
        ] = historyRules[i]["MBLsheet_conditionformat_save"];
      ...
      Store.MBLsheetfile[index]["MBLsheet_alternateformat_save"] = $.extend(
        true,
        [],
        historyRules
      );
      ...
      if (ctr.config["borderInfo"] == null) {
        ...
        server.saveParam("cg", ctr.sheetIndex, ctr.config["borderInfo"], {
          k: "borderInfo",
        });
      }
      ...
      MBLsheetPostil.ref(ctr.data, ctr.rc);
      ...
      for (let i = 0; i < ctr.rc.length; i++) {
        ...
        if (ctr.data[r][c] != null && ctr.data[r][c].ps != null) {
          MBLsheetPostil.buildPs(r, c, ctr.data[r][c].ps);
        } else {
      ...
      server.saveParam("all", ctr.currentSheetIndex, ctr.zoomRatio, {
        k: "zoomRatio",
      });
      zoomNumberDomBind();
      zoomRefreshView();
    ...
    cleargridelement(e);
    if (ctr.range) {
      Store.MBLsheet_select_save = ctr.range;
  ...
  undo: function () {
    ...
    if (ctr.type == "datachange") {
      ...
      if (ctr.ctrlType == "resizeR") {
        server.saveParam("cg", ctr.sheetIndex, ctr.curconfig["rowlen"], {
          k: "rowlen",
        });
      } else if (ctr.ctrlType == "resizeC") {
        server.saveParam("cg", ctr.sheetIndex, ctr.curconfig["columnlen"], {
          k: "columnlen",
        });
      }
      ...
      //config
      Store.config = ctr.curconfig;
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].config = ctr.curconfig;

      server.saveParam("cg", ctr.sheetIndex, ctr.curconfig["rowhidden"], {
        k: "rowhidden",
      });
      ...
      server.saveParam("cg", ctr.sheetIndex, ctr.curconfig["colhidden"], {
        k: "colhidden",
      });
      ...
      Store.MBLsheet_select_save = [ctr.filter_save];
      Store.filterchage = false;
      createFilter();
      Store.filterchage = true;
      server.saveParam("all", ctr.sheetIndex, ctr.filter_save, {
        k: "filter_select",
      });
    } else if (ctr.type == "pivotTable_change") {
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].pivotTable =
      ...
      if (ctr.order == 0) {
        sheetmanage.changeSheetExec(Store.MBLsheetfile[0].index);
      } else {
        sheetmanage.changeSheetExec(Store.MBLsheetfile[ctr.order - 1].index);
      }

      $("#MBLsheet-input-box").removeAttr("style");
      $("#MBLsheet-sheet-list, #MBLsheet-rightclick-sheet-menu").hide();
    } else if (ctr.type == "sheetName") {
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].name = ctr.txt;
      $("#MBLsheet-sheets-item" + ctr.sheetIndex)
        .find(".MBLsheet-sheets-item-name")
        .html(ctr.txt);
      ...
      Store.MBLsheetfile[getSheetIndex(ctr.sheetIndex)].color = ctr.color;
      ...
      for (let i = 0; i < currentRules.length; i++) {
        ...
        Store.MBLsheetfile[getSheetIndex(sheetIndex)][
          "MBLsheet_conditionformat_save"
        ] = currentRules[i]["MBLsheet_conditionformat_save"];
      ...
      server.saveParam("cg", ctr.sheetIndex, ctr.curconfig["borderInfo"], {
        k: "borderInfo",
      });
      ...
      MBLsheetPostil.ref(ctr.curdata, ctr.rc);
      ...
      for (let i = 0; i < ctr.rc.length; i++) {
        ...
        if (ctr.curdata[r][c] != null && ctr.curdata[r][c].ps != null) {
          MBLsheetPostil.buildPs(r, c, ctr.curdata[r][c].ps);
        } else {
          MBLsheetPostil.buildPs(r, c, null);
        }
      }
      ...
      server.saveParam("all", ctr.currentSheetIndex, ctr.curZoomRatio, {
        k: "zoomRatio",
      });
      zoomNumberDomBind();
      zoomRefreshView();
