...
const MBLsheetFreezen = {
  freezenHorizontalHTML:
    '<div id="MBLsheet-freezebar-horizontal" class="MBLsheet-freezebar" tabindex="0"><div class="MBLsheet-freezebar-handle MBLsheet-freezebar-horizontal-handle" ><div class="MBLsheet-freezebar-handle-bar MBLsheet-freezebar-horizontal-handle-title" ></div><div class="MBLsheet-freezebar-handle-bar MBLsheet-freezebar-horizontal-handle-bar" ></div></div><div class="MBLsheet-freezebar-drop MBLsheet-freezebar-horizontal-drop" ><div class="MBLsheet-freezebar-drop-bar MBLsheet-freezebar-horizontal-drop-title" ></div><div class="MBLsheet-freezebar-drop-bar MBLsheet-freezebar-horizontal-drop-bar" >&nbsp;</div></div></div>',
  freezenVerticalHTML:
    '<div id="MBLsheet-freezebar-vertical" class="MBLsheet-freezebar" tabindex="0"><div class="MBLsheet-freezebar-handle MBLsheet-freezebar-vertical-handle" ><div class="MBLsheet-freezebar-handle-bar MBLsheet-freezebar-vertical-handle-title" ></div><div class="MBLsheet-freezebar-handle-bar MBLsheet-freezebar-vertical-handle-bar" ></div></div><div class="MBLsheet-freezebar-drop MBLsheet-freezebar-vertical-drop" ><div class="MBLsheet-freezebar-drop-bar MBLsheet-freezebar-vertical-drop-title" ></div><div class="MBLsheet-freezebar-drop-bar MBLsheet-freezebar-vertical-drop-bar" >&nbsp;</div></div></div>',
  ...
  cancelFreezenVertical: function (sheetIndex) {
            ...
            style="user-select: none;">
                ...
                style="user-select: none;">
                    <div class="MBLsheet-icon MBLsheet-inline-block " style="user-select: none;">
                        <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-function iconfont MBLsheet-iconfont-dongjie1"
                        style="user-select: none;">
                        </div>
                    </div>
    ...
    $("#MBLsheet-freezen-btn-horizontal").html(freezeHTML);

    $("#MBLsheet-freezen-btn-vertical").html(
      '<i class="fa fa-indent"></i> ' + locale_freezen.freezenColumn
    );
    _this.freezenverticaldata = null;
    let isvertical = $("#MBLsheet-freezebar-vertical").is(":visible");
    $("#MBLsheet-freezebar-vertical").hide();

    if (sheetIndex == null) {
      sheetIndex = Store.currentSheetIndex;
    }

    let currentSheet = Store.MBLsheetfile[getSheetIndex(sheetIndex)];
  ...
  createFreezenVertical: function (freezenverticaldata, left) {
    ...
    if (_this.initialVertical) {
      ...
      $("#MBLsheet-grid-window-1").append(_this.freezenVerticalHTML);

      $("#MBLsheet-freezebar-vertical")
        .find(".MBLsheet-freezebar-vertical-drop")
        .hover(
          function () {
            $(this).parent().addClass("MBLsheet-freezebar-hover");
          },
          function () {
            $(this).parent().removeClass("MBLsheet-freezebar-hover");
          }
        );
      ...
      $("#MBLsheet-freezebar-vertical")
        ...
        .mousedown(function () {
          ...
          _this.windowWidth = $("#MBLsheet-grid-window-1").width();

          $(this).parent().addClass("MBLsheet-freezebar-active");
          $("#MBLsheet-freezebar-vertical")
    ...
    // $("#MBLsheet-freezen-btn-horizontal").html('<i class="MBLsheet-icon-img-container iconfont MBLsheet-iconfont-dongjie1"></i> '+locale().freezen.freezenCancel);
  ...
  scrollFreezen: function () {
    ...
    if (row_focus == Store.MBLsheet_select_save[0]["row"][0]) {
      row = Store.MBLsheet_select_save[0]["row"][1];
    } else if (row_focus == Store.MBLsheet_select_save[0]["row"][1]) {
      row = Store.MBLsheet_select_save[0]["row"][0];
    }

    let column;
    let column_focus = Store.MBLsheet_select_save[0]["column_focus"];
    if (column_focus == Store.MBLsheet_select_save[0]["column"][0]) {
      column = Store.MBLsheet_select_save[0]["column"][1];
    } else if (column_focus == Store.MBLsheet_select_save[0]["column"][1]) {
      column = Store.MBLsheet_select_save[0]["column"][0];
    }
  ...
  cancelFreezenHorizontal: function (sheetIndex) {
    ...
    // $("#MBLsheet-freezen-btn-horizontal").html('<i class="MBLsheet-icon-img-container iconfont MBLsheet-iconfont-dongjie1"></i> '+locale().freezen.default);
    ...
    let ishorizontal = $("#MBLsheet-freezebar-horizontal").is(":visible");
    $("#MBLsheet-freezebar-horizontal").hide();
  ...
  createFreezenHorizontal: function (freezenhorizontaldata, top) {
    ...
    if (_this.initialHorizontal) {
      ...
      $("#MBLsheet-grid-window-1").append(_this.freezenHorizontalHTML);
      ...
      $("#MBLsheet-freezebar-horizontal")
        ...
        .mousedown(function () {
          ...
          _this.windowHeight = $("#MBLsheet-grid-window-1").height();
    ...
    if (freezenhorizontaldata == null) {
      ...
      if (_this.freezenRealFirstRowColumn) {
        ...
        dataset_row_st = MBLsheet_searcharray(Store.visibledatarow, scrollTop);
        if (dataset_row_st == -1) {
          dataset_row_st = 0;
        }
    ...
    // $("#MBLsheet-freezen-btn-horizontal").html('<i class="fa fa-list-alt"></i> '+locale().freezen.freezenCancel);
  ...
  createCanvas: function (id, width, height, left, top) {
    let c = $("<canvas/>")
      .appendTo("#MBLsheet-grid-window-1")
      .attr({
        id: id,
  ...
  removeAssistCanvas: function () {
    $("#MBLsheet-grid-window-1 > canvas")
      .not($("#MBLsheetTableContent"))
      .remove();
    $("#MBLsheet-cell-selected").css("z-index", 15);
  },
  scrollAdapt: function () {
    ...
    //有冻结时 图表框 滚动适应
    if (
      $("#MBLsheet-cell-main .MBLsheet-data-visualization-chart").length > 0
    ) {
      // 当前sheet有图表才需要滚动适应
      const chart =
        Store.MBLsheetfile[getSheetIndex(Store.currentSheetIndex)]["chart"];
    ...
    //有冻结时 批注框 滚动适应
    if ($("#MBLsheet-postil-showBoxs .MBLsheet-postil-show").length > 0) {
      _this.scrollAdaptOfpostil();
    }

    //有冻结时 下拉选区图标 滚动适应
    if ($("#MBLsheet-dropCell-icon").length > 0) {
  ...
  scrollAdaptOfselect: function () {
    ...
    if ($("#MBLsheet-row-count-show").is(":visible")) {
      $("#MBLsheet-row-count-show").hide();
    }

    if ($("#MBLsheet-column-count-show").is(":visible")) {
    ...
    if (
      ...
      for (let s = 0; s < Store.MBLsheet_select_save.length; s++) {
        let obj = $.extend(true, {}, Store.MBLsheet_select_save[s]);
        ...
        if (r1 >= freezen_rowindex) {
          ...
          if (top_move + height_move < freezenTop + offTop) {
            ...
            $("#MBLsheet-cell-selected-boxs")
              .find(".MBLsheet-cell-selected")
              .eq(s)
              .show()
              .css({
        ...
        if (s == Store.MBLsheet_select_save.length - 1) {
          ...
          if (top >= freezenTop) {
            if (top + height < freezenTop + offTop) {
              ...
              $("#MBLsheet-cell-selected-focus").show().css({
                top: top,
                height: height,
              });
            }
  ...
  scrollAdaptOfchart: function () {
    ...
    if (
      ...
      $("#MBLsheet-cell-main .MBLsheet-data-visualization-chart").each(
        function (i, e) {
          let x = $(e).position();
          let width = $(e).width();
          let height = $(e).height();
  ...
  scrollAdaptOfpostil: function () {
    ...
    if (
      ...
      $("#MBLsheet-postil-showBoxs .MBLsheet-postil-show").each(function (
        i,
        e
      ) {
        let id = $(e).attr("id");
        ...
        let postil_width =
          postil["width"] == null
            ? MBLsheetPostil.defaultWidth
            : postil["width"];
        let postil_height =
          postil["height"] == null
            ? MBLsheetPostil.defaultHeight
            : postil["height"];
        ...
        let size = MBLsheetPostil.getArrowCanvasSize(
          postil_left,
          postil_top,
          toX,
          toY
            ...
            if (postil_left < freezenLeft + offLeft) {
              if (postil_left + postil_width <= freezenLeft + offLeft) {
                ...
                $(e)
                  .show()
                  .find(".formulaInputFocus")
                  .css("margin-left", -(freezenLeft + offLeft - postil_left));
                $(e).show().find(".arrowCanvas").hide();
  ...
  scrollAdaptOfdpicon: function () {
    ...
    let copy_r = MBLsheetDropCell.copyRange["row"][1],
      copy_c = MBLsheetDropCell.copyRange["column"][1];

    let apply_r = MBLsheetDropCell.applyRange["row"][1],
      apply_c = MBLsheetDropCell.applyRange["column"][1];
    ...
    if (
      ...
      let freezen_rowindex = _this.freezenhorizontaldata[1];
      let offsetRow = MBLsheet_searcharray(
        _this.freezenhorizontaldata[3],
        $("#MBLsheet-cell-main").scrollTop() - _this.freezenhorizontaldata[2]
      );
      let freezen_colindex = _this.freezenverticaldata[1];
      let offsetColumn = MBLsheet_searcharray(
        _this.freezenverticaldata[3],
        $("#MBLsheet-cell-main").scrollLeft() - _this.freezenverticaldata[2]
      );
      ...
      if (row_index >= freezen_rowindex && col_index >= freezen_colindex) {
        if (
          ...
          $("#MBLsheet-dropCell-icon").hide();
        } else {
          $("#MBLsheet-dropCell-icon").show();
        }
        ...
        if (row_index < freezen_rowindex + offsetRow - 1) {
          ...
          $("#MBLsheet-dropCell-icon").show().css("left", col);
        }
        ...
        if (col_index < freezen_colindex + offsetColumn - 1) {
          ...
          $("#MBLsheet-dropCell-icon").show().css("top", row);
        }
      } else {
        let row = rowLocationByIndex(row_index + offsetRow)[1],
          col = colLocationByIndex(col_index + offsetColumn)[1];

        $("#MBLsheet-dropCell-icon").show().css({ left: col, top: row });
      }
    } else if (_this.freezenhorizontaldata != null) {
      let freezen_rowindex = _this.freezenhorizontaldata[1];
      let offsetRow = MBLsheet_searcharray(
  ...
  scrollAdaptOffilteroptions: function () {
    ...
    if (
      ...
      let freezen_rowindex = _this.freezenhorizontaldata[1];
      let freezen_top =
        _this.freezenhorizontaldata[0] + $("#MBLsheet-cell-main").scrollTop();
      ...
      $(
        ...
        if (row_index >= freezen_rowindex && col_index >= freezen_colindex) {
          ...
          $(e)
            ...
            .css({
              left: left,
              top: top + $("#MBLsheet-cell-main").scrollTop(),
            });
        }
      });
    } else if (_this.freezenhorizontaldata != null) {
      ...
      $("#MBLsheet-filter-options-sheet" + Store.currentSheetIndex).empty();
      createFilterOptions(
  ...
  saveFrozen: function (operate, order, focus = {}) {
    ...
    // store frozen
    Store.MBLsheetfile[order]["frozen"] = frozen[operate];
  },
...
export default MBLsheetFreezen;
