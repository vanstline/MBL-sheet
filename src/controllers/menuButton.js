...
import MBLsheetMoreFormat from "./moreFormat";
...
import MBLsheetSearchReplace from "./searchReplace";
import MBLsheetLocationCell from "./locationCell";
...
import { jfrefreshgrid, MBLsheetrefreshgrid } from "../global/refresh";
...
import { getSheetIndex, getRangetxt, get_MBLsheetfile } from "../methods/get";
import { setMBLsheetfile } from "../methods/set";
import {
  isInlineStringCell,
  isInlineStringCT,
...
const menuButton = {
  menu: '<div class="MBLsheet-cols-menu MBLsheet-rightgclick-menu MBLsheet-menuButton ${subclass} MBLsheet-mousedown-cancel" id="MBLsheet-icon-${id}-menuButton">${item}</div>',
  // "item": '<div itemvalue="${value}" itemname="${name}" class="MBLsheet-cols-menuitem ${sub} MBLsheet-mousedown-cancel"><div class="MBLsheet-cols-menuitem-content MBLsheet-mousedown-cancel" style="padding: 3px 0px 3px 1px;"><span style="margin-right:3px;width:13px;display:inline-block;" class="icon MBLsheet-mousedown-cancel"></span> ${name} <span class="MBLsheet-submenu-arrow MBLsheet-mousedown-cancel" style="user-select: none;">${example}</span></div></div>',
  item: '<div itemvalue="${value}" itemname="${name}" class="MBLsheet-cols-menuitem ${sub} MBLsheet-mousedown-cancel"><div class="MBLsheet-cols-menuitem-content MBLsheet-mousedown-cancel" style="padding: 3px 0px 3px 1px;"><span style="margin-right:3px;width:13px;display:inline-block;" class="icon MBLsheet-mousedown-cancel"></span> ${name} <span class="MBLsheet-submenu-arrow MBLsheet-mousedown-cancel ${iconClass}" style="user-select: none;">${example}</span></div></div>',
  split:
    '<div class="MBLsheet-menuseparator MBLsheet-mousedown-cancel" role="separator"></div>',
  color:
    '<div class="MBLsheet-cols-menu MBLsheet-rightgclick-menu MBLsheet-rightgclick-menu-sub MBLsheet-mousedown-cancel MBLsheet-menuButton ${sub}" id="${id}"><div class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel MBLsheet-color-reset"><div class="MBLsheet-cols-menuitem-content MBLsheet-mousedown-cancel">${resetColor}</div></div> <div class="MBLsheet-mousedown-cancel"> <div class="MBLsheet-mousedown-cancel"> <input type="text" class="MBLsheet-color-selected" /> </div> </div> <div class="MBLsheet-menuseparator MBLsheet-mousedown-cancel" role="separator"></div> ${coloritem}</div>',
  coloritem:
    '<div class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel ${class}"><div class="MBLsheet-cols-menuitem-content MBLsheet-mousedown-cancel">${name}</div></div>',
  subcolor:
    '<div id="MBLsheet-icon-${id}-menuButton" class="MBLsheet-cols-menu MBLsheet-rightgclick-menu MBLsheet-rightgclick-menu-sub MBLsheet-menuButton-sub MBLsheet-mousedown-cancel"> <div class="MBLsheet-mousedown-cancel"> <div class="MBLsheet-mousedown-cancel"> <input type="text" class="MBLsheet-color-selected" /> </div> </div></div>',
  rightclickmenu: null,
  submenuhide: {},
  focus: function ($obj, value) {
    if ($obj.attr("id") == "MBLsheet-icon-font-family-menuButton") {
    ...
    $obj.find(".MBLsheet-cols-menuitem").find("span.icon").html("");
    if (value == null) {
      $obj
        .find(".MBLsheet-cols-menuitem")
        .eq(0)
        .find("span.icon")
        .html('<i class="fa fa-check MBLsheet-mousedown-cancel"></i>');
    } else {
      $obj
        .find(".MBLsheet-cols-menuitem[itemvalue='" + value + "']")
        .find("span.icon")
        .html('<i class="fa fa-check MBLsheet-mousedown-cancel"></i>');
    }
  },
    ...
    for (let i = 0; i < itemdata.length; i++) {
      ...
      if (item.value == "split") {
        ...
        if (item.example == "more") {
          // itemset += replaceHtml(_this.item, {"value": item.value, "name": item.text, "example": "►", "sub": "MBLsheet-cols-submenu"});
          itemset += replaceHtml(_this.item, {
            value: item.value,
            name: item.text,
            example: "",
            sub: "MBLsheet-cols-submenu",
            iconClass: "iconfont MBLsheet-iconfont-youjiantou",
          });
        } else {
          itemset += replaceHtml(_this.item, {
            value: item.value,
  ...
  cancelPaintModel: function () {
    ...
    $("#MBLsheet-sheettable_0").removeClass("MBLsheetPaintCursor");

    if (Store.MBLsheet_copy_save["dataSheetIndex"] == Store.currentSheetIndex) {
      Store.MBLsheet_selection_range = [];
      selectionCopyShow();
    } else {
      Store.MBLsheetfile[
        getSheetIndex(Store.MBLsheet_copy_save["dataSheetIndex"])
      ].MBLsheet_selection_range = [];
    }

    Store.MBLsheet_copy_save = {};

    _this.MBLsheetPaintModelOn = false;
    $("#MBLsheetpopover").fadeOut(200, function () {
      $("#MBLsheetpopover").remove();
    });
  },
  MBLsheetPaintModelOn: false,
  MBLsheetPaintSingle: false,
  initialMenuButton: function () {
    let _this = this;

    //格式刷
    $("#MBLsheet-icon-paintformat").click(function (e) {
      // *如果禁止前台编辑，则中止下一步操作
      if (!checkIsAllowEdit()) {
      ...
      if (
        ...
        return;
      } else if (Store.MBLsheet_select_save.length > 1) {
        if (isEditMode()) {
      ...
      let r1 = Store.MBLsheet_select_save[0].row[0],
        r2 = Store.MBLsheet_select_save[0].row[1];

      let c1 = Store.MBLsheet_select_save[0].column[0],
        c2 = Store.MBLsheet_select_save[0].column[1];
      ...
      $("#MBLsheet-sheettable_0").addClass("MBLsheetPaintCursor");

      Store.MBLsheet_selection_range = [
        {
          row: Store.MBLsheet_select_save[0].row,
          column: Store.MBLsheet_select_save[0].column,
        },
      ];
      ...
      for (
        let r = Store.MBLsheet_select_save[0].row[0];
        r <= Store.MBLsheet_select_save[0].row[1];
        r++
      ) {
        if (
        ...
        for (
          let c = Store.MBLsheet_select_save[0].column[0];
          c <= Store.MBLsheet_select_save[0].column[1];
          c++
        ) {
      ...
      _this.MBLsheetPaintModelOn = true;
      _this.MBLsheetPaintSingle = true;
    });
    $("#MBLsheet-icon-paintformat").dblclick(function () {
      // *如果禁止前台编辑，则中止下一步操作
      if (!checkIsAllowEdit()) {
        tooltip.info("", locale().pivotTable.errorNotAllowEdit);
        return;
      ...
      _this.MBLsheetPaintModelOn = true;
      _this.MBLsheetPaintSingle = false;
    });

    //货币格式
    $("#MBLsheet-icon-currency").click(function () {
    ...
    //百分比
    $("#MBLsheet-icon-percent").click(function () {
      let d = editor.deepCopyFlowData(Store.flowdata); //取数据
    ...
    //减少小数位数
    $("#MBLsheet-icon-fmt-decimal-decrease").click(function () {
      let d = editor.deepCopyFlowData(Store.flowdata); //取数据
      let row_index = Store.MBLsheet_select_save[0]["row_focus"],
        col_index = Store.MBLsheet_select_save[0]["column_focus"];
    ...
    //增加小数位数
    $("#MBLsheet-icon-fmt-decimal-increase").click(function () {
      let d = editor.deepCopyFlowData(Store.flowdata); //取数据
      let row_index = Store.MBLsheet_select_save[0]["row_focus"],
        col_index = Store.MBLsheet_select_save[0]["column_focus"];
      let foucsStatus = _this.checkstatus(d, row_index, col_index, "ct");
    ...
    //更多格式
    $("#MBLsheet-icon-fmt-other").click(function () {
      ...
      if ($menuButton.length == 0) {
        ...
        let submenu = replaceHtml(_this.menu, {
          id: "fmtOtherSelf",
          item: subitemset,
          subclass: "MBLsheet-menuButton-sub",
        });

        //MBLsheet-icon-fmt-other-menuButton_sub
        $("body").append(menu + submenu);
        $menuButton = $("#" + menuButtonId).width(250);
        _this.focus($menuButton);

        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          $menuButton.hide();
          MBLsheetContainerFocus();
          ...
          $("#MBLsheet-icon-fmt-other")
            .find(".MBLsheet-toolbar-menu-button-caption")
            .html(" " + itemname + " ");
        ...
        //更多格式
        $("#MBLsheet-icon-fmtOtherSelf-menuButton")
          .find(".MBLsheet-cols-menuitem")
          .click(function () {
            $menuButton.hide();
            $("#MBLsheet-icon-fmtOtherSelf-menuButton").hide();
            MBLsheetContainerFocus();

            let itemvalue = $(this).attr("itemvalue");

            MBLsheetMoreFormat.createDialog(itemvalue);
            MBLsheetMoreFormat.init();
          });
      } else {
        const text = $(this)
          .find(".MBLsheet-toolbar-menu-button-caption")
    ...
    $("#MBLsheet-icon-font-family")
      ...
      .click(function () {
        ...
        if ($menuButton.length == 0) {
          ...
          //     let ret = {};
          //     ret.value = a;
          //     ret.text = "<span class='MBLsheet-mousedown-cancel' style='font-size:11px;font-family:"+fItem+"'>"+fItem+"</span>";
          //     ret.example = "";
          //     itemdata.push(ret);
          // }
          ...
          $menuButton.on("click", ".MBLsheet-cols-menuitem", function () {
            $menuButton.hide();
            MBLsheetContainerFocus();
      ...
      .click(function () {
        ...
        if ($menuButton.length == 0) {
          ...
          let coloritem = replaceHtml(_this.coloritem, {
            class: "MBLsheet-icon-alternateformat",
            name: locale_toolbar.alternatingColors + "...",
          });
          ...
          $("#" + menuButtonId)
            ...
            .spectrum({
              ...
              color: MBLsheetConfigsetting.defaultTextColor,
              ...
              change: function (color) {
                ...
                let oldcolor = null;
                // $("#MBLsheet-icon-text-color .MBLsheet-color-menu-button-indicator").css("border-bottom-color", color);
                // 下边框换成了一个DIV
                $("#MBLsheet-icon-text-color .text-color-bar").css(
                  "background-color",
                  color
                );
                $("#MBLsheet-icon-text-color").attr("color", color);
          ...
          $menuButton.find(".MBLsheet-color-reset").click(function () {
            $menuButton.hide();
            MBLsheetContainerFocus();

            let $input = $("#" + menuButtonId).find(".MBLsheet-color-selected");
            $input.val("#000000");
            $("#MBLsheet-icon-text-color").attr("color", null);
            $input.spectrum("set", "#000000");
            $(
              "#MBLsheet-icon-text-color .MBLsheet-color-menu-button-indicator"
          ...
          //交替颜色
          $menuButton.find(".MBLsheet-icon-alternateformat").click(function () {
            $menuButton.hide();
            MBLsheetContainerFocus();
            ...
            let range = $.extend(true, {}, Store.MBLsheet_select_save[0]);
    ...
    //背景颜色
    $("#MBLsheet-icon-cell-color").click(function () {
      let d = editor.deepCopyFlowData(Store.flowdata);
      let color = $(this).attr("color");
      if (color == null) {
        color = "#ffffff";
    ...
    $("#MBLsheet-icon-cell-color-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        $("#" + menuButtonId)
          ...
          .spectrum({
            ...
            maxPaletteSize: 8,
            maxSelectionSize: 8,
            color: MBLsheetConfigsetting.defaultCellColor,
            cancelText: locale_button.cancel,
            chooseText: locale_button.confirm,
            togglePaletteMoreText: locale_toolbar.customColor,
            togglePaletteLessText: locale_toolbar.collapse,
            ...
            change: function (color) {
              ...
              let oldcolor = null;
              // $("#MBLsheet-icon-cell-color .MBLsheet-color-menu-button-indicator").css("border-bottom-color", color);
              // 下边框换成了一个DIV
              $("#MBLsheet-icon-cell-color .text-color-bar").css(
                "background-color",
                color
              );

              $("#MBLsheet-icon-cell-color").attr("color", color);
              let d = editor.deepCopyFlowData(Store.flowdata);
              _this.updateFormat(d, "bg", color);
        ...
        $menuButton.find(".MBLsheet-color-reset").click(function () {
          ...
          let $input = $("#" + menuButtonId).find(".MBLsheet-color-selected");
          $input.val("#ffffff");
          $("#MBLsheet-icon-cell-color").attr("color", null);
          $input.spectrum("set", "#ffffff");
          $(
            "#MBLsheet-icon-cell-color .MBLsheet-color-menu-button-indicator"
    ...
    //字体大小
    let MBLsheet_fs_setTimeout = null;
    $("#MBLsheet-icon-font-size")
      .mousedown(function (e) {
      ...
      .click(function () {
        ...
        if ($menuButton.length == 0) {
          ...
          $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
            ...
            let $t = $(this),
              itemvalue = $t.attr("itemvalue"),
              $input = $("#MBLsheet-icon-font-size input");
            $("#MBLsheet-icon-font-size").attr("itemvalue", itemvalue);
            _this.focus($menuButton, itemvalue);
            $input.val(itemvalue);
            ...
            clearTimeout(MBLsheet_fs_setTimeout);
          });
        }
        ...
        let defualtvalue = $("#MBLsheet-icon-font-size").attr("itemvalue");
      ...
      .find("input.MBLsheet-toolbar-textinput")
      .keydown(function (e) {
        hideMenuByCancel(e);
        e.stopPropagation();
      })
      .keyup(function (e) {
        ...
        let $menuButton = $("#MBLsheet-icon-font-size-menuButton");
        _this.focus($menuButton, itemvalue);

        let d = editor.deepCopyFlowData(Store.flowdata);
        _this.updateFormat(d, "fs", itemvalue);

        MBLsheet_fs_setTimeout = setTimeout(function () {
          $menuButton.hide();
          $this.blur();
        }, 200);
      });
    ...
    //边框设置
    $("#MBLsheet-icon-border-all").click(function () {
      // *如果禁止前台编辑，则中止下一步操作
      if (!checkIsAllowEdit()) {
        tooltip.info("", locale().pivotTable.errorNotAllowEdit);
        return;
      ...
      let subcolormenuid = "MBLsheet-icon-borderColor-menuButton";
      let color = $("#" + subcolormenuid)
        .find(".MBLsheet-color-selected")
        .val();
      let style = $("#MBLsheetborderSizepreview").attr("itemvalue");
    ...
    $("#MBLsheet-icon-border-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        let itemdata = [
          {
            ...
            value: "border-top",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-top iconfont MBLsheet-iconfont-shangbiankuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderBottom,
            value: "border-bottom",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-bottom iconfont MBLsheet-iconfont-xiabiankuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderLeft,
            value: "border-left",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-left iconfont MBLsheet-iconfont-zuobiankuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderRight,
            value: "border-right",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-right iconfont MBLsheet-iconfont-youbiankuang" style="user-select: none;"> </div> </div>',
          },
          { text: "", value: "split", example: "" },
          {
            ...
            value: "border-none",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-none iconfont MBLsheet-iconfont-wubiankuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderAll,
            value: "border-all",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-all iconfont MBLsheet-iconfont-quanjiabiankuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderOutside,
            value: "border-outside",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-outside iconfont MBLsheet-iconfont-sizhoujiabiankuang" style="user-select: none;"> </div> </div>',
          },
          { text: "", value: "split", example: "" },
          {
            ...
            value: "border-inside",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-inside iconfont MBLsheet-iconfont-neikuangxian" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderHorizontal,
            value: "border-horizontal",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-horizontal iconfont MBLsheet-iconfont-neikuanghengxian" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_border.borderVertical,
            value: "border-vertical",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block MBLsheet-material-icon MBLsheet-mousedown-cancel" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-border-vertical iconfont MBLsheet-iconfont-neikuangshuxian" style="user-select: none;"> </div> </div>',
          },
          { text: "", value: "split", example: "" },
          {
            text:
              "<span id='MBLsheet-icon-borderColor-linecolor' class='MBLsheet-mousedown-cancel' style='border-bottom:3px solid #000;'>" +
          ...
          },
          {
            text:
              ...
              "<img id='MBLsheetborderSizepreview' width=100 height=10 src='data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==' style='position:absolute;bottom:-5px;right:0px;width:100px;height:10px;'>",
            value: "borderSize",
            example: "more",
          },
        ];
        ...
        let submenuid = "MBLsheet-icon-borderSize-menuButton";
        let subcolormenuid = "MBLsheet-icon-borderColor-menuButton";
        let colormenu = replaceHtml(_this.color, {
          id: subcolormenuid,
          coloritem: "",
          colorself: "",
          sub: "MBLsheet-menuButton-sub",
          resetColor: locale_toolbar.resetColor,
        });
        ...
        $("#" + submenuid + " canvas").each(function (i) {
          ...
          let canvasborder = $(this)
            .addClass("MBLsheet-mousedown-cancel")
            .get(0)
            .getContext("2d");
        ...
        $("#" + submenuid + " .MBLsheet-cols-menuitem").click(function () {
          $("#" + submenuid).hide();
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          $("#MBLsheet-icon-border-all").attr("type", itemvalue);

          let $icon = $("#MBLsheet-icon-border-all").find(
            ".MBLsheet-icon-img-container"
          );
          ...
          $icon
            .removeAttr("class")
            .addClass(
              "MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-" +
                itemvalue +
        ...
        $("#" + subcolormenuid)
          ...
          .spectrum({
            ...
            change: function (color) {
              ...
              let oldcolor = null;
              $("#MBLsheet-icon-borderColor-linecolor").css(
                "border-bottom-color",
                color
              );
              $("#" + subcolormenuid)
        ...
        $("#" + subcolormenuid)
          .find(".MBLsheet-color-reset")
          .click(function () {
            let $input = $("#" + subcolormenuid).find(
              ".MBLsheet-color-selected"
            );
            $input.val("#000");
            $("#MBLsheet-icon-cell-color").attr("color", null);
    ...
    //合并单元格
    $("#MBLsheet-icon-merge-button").click(function () {
      const _locale = locale();
    ...
    $("#MBLsheet-icon-merge-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
    ...
    //水平对齐
    $("#MBLsheet-icon-align").click(function () {
      let itemvalue = $("#MBLsheet-icon-align").attr("type");
      if (itemvalue == null) {
        itemvalue = "left";
      }
    ...
    $("#MBLsheet-icon-align-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        let itemdata = [
          {
            ...
            value: "left",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-align-left iconfont MBLsheet-iconfont-wenbenzuoduiqi" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_align.center,
            value: "center",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-align-center iconfont MBLsheet-iconfont-wenbenjuzhongduiqi" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_align.right,
            value: "right",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-align-right iconfont MBLsheet-iconfont-wenbenyouduiqi" style="user-select: none;"> </div> </div>',
          },
        ];
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          let $icon = $("#MBLsheet-icon-align")
            .attr("type", itemvalue)
            .find(".MBLsheet-icon-img-container");

          // add iconfont
          $icon
            .removeAttr("class")
            .addClass(
              "MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-align-" +
                itemvalue +
                iconfontObject[itemvalue]
            );
    ...
    //垂直对齐
    $("#MBLsheet-icon-valign").click(function () {
      let itemvalue = $("#MBLsheet-icon-valign").attr("type");
      if (itemvalue == null) {
        itemvalue = "bottom";
      }
    ...
    $("#MBLsheet-icon-valign-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      const _locale = locale();
      const locale_align = _locale.align;
      if ($menuButton.length == 0) {
        let itemdata = [
          {
            ...
            value: "top",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-valign-top iconfont MBLsheet-iconfont-dingbuduiqi" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_align.middle,
            value: "middle",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-valign-middle iconfont MBLsheet-iconfont-shuipingduiqi" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_align.bottom,
            value: "bottom",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-valign-bottom iconfont MBLsheet-iconfont-dibuduiqi" style="user-select: none;"> </div> </div>',
          },
        ];
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          $icon
            .removeAttr("class")
            .addClass(
              "MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-valign-" +
                itemvalue +
                iconfontObject[itemvalue]
            );
    ...
    //文本换行
    $("#MBLsheet-icon-textwrap-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        let itemdata = [
          {
            ...
            value: "overflow",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-textwrap-overflow iconfont MBLsheet-iconfont-yichu1" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_textWrap.wrap,
            value: "wrap",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-textwrap-wrap iconfont MBLsheet-iconfont-zidonghuanhang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_textWrap.clip,
            value: "clip",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-textwrap-clip iconfont MBLsheet-iconfont-jieduan" style="user-select: none;"> </div> </div>',
          },
        ];
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          let $icon = $("#MBLsheet-icon-textwrap")
            .attr("type", itemvalue)
            .find(".MBLsheet-icon-img-container");
          ...
          $icon
            .removeAttr("class")
            .addClass(
              "MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-textwrap-" +
                itemvalue +
                iconfontObject[itemvalue]
            );
    ...
    //文本旋转
    $("#MBLsheet-icon-rotation-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        let itemdata = [
          {
            ...
            value: "none",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-none iconfont MBLsheet-iconfont-wuxuanzhuang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_rotation.angleup,
            value: "angleup",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-angleup iconfont MBLsheet-iconfont-xiangshangqingxie" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_rotation.angledown,
            value: "angledown",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-angledown iconfont MBLsheet-iconfont-xiangxiaqingxie" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_rotation.vertical,
            value: "vertical",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-vertical iconfont MBLsheet-iconfont-shupaiwenzi" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_rotation.rotationUp,
            value: "rotation-up",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-rotation-up iconfont MBLsheet-iconfont-wenbenxiangshang" style="user-select: none;"> </div> </div>',
          },
          {
            text: locale_rotation.rotationDown,
            value: "rotation-down",
            example:
              '<div class="MBLsheet-icon MBLsheet-inline-block" style="user-select: none;opacity:1;"> <div aria-hidden="true" class="MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-rotation-down iconfont MBLsheet-iconfont-xiangxia90" style="user-select: none;"> </div> </div>',
          },
        ];
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          let $icon = $("#MBLsheet-icon-rotation")
            .attr("type", itemvalue)
            .find(".MBLsheet-icon-img-container");
          ...
          $icon
            .removeAttr("class")
            .addClass(
              "MBLsheet-icon-img-container MBLsheet-icon-img MBLsheet-icon-rotation-" +
                itemvalue +
                iconfontObject[itemvalue]
            );
    ...
    //冻结行列
    $("#MBLsheet-icon-freezen-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          // store frozen
          MBLsheetFreezen.saveFrozen(itemvalue);
          ...
          if (itemvalue == "freezenRow") {
            ...
            // let freezenhorizontaldata = [Store.visibledatarow[row_st], row_st + 1, scrollTop, MBLsheetFreezen.cutVolumn(Store.visibledatarow, row_st + 1), top];
            ...
            // let freezenverticaldata = [Store.visibledatacolumn[col_st], col_st + 1, scrollLeft, MBLsheetFreezen.cutVolumn(Store.visibledatacolumn, col_st + 1), left];
            ...
            if (MBLsheetFreezen.freezenRealFirstRowColumn) {
              ...
              let scrollTop = $("#MBLsheet-cell-main").scrollTop();
              let row_st = MBLsheet_searcharray(
            ...
            let rangeTop =
              Store.MBLsheet_select_save[Store.MBLsheet_select_save.length - 1]
                .top;
            if (
              MBLsheetFreezen.freezenRealFirstRowColumn &&
              rangeTop > $("#MBLsheet-cell-main").height()
            ) {
              return tooltip.info(
            ...
            if (
              MBLsheetFreezen.freezenRealFirstRowColumn &&
              rangeLeft > $("#MBLsheet-cell-main").width()
            ) {
              return tooltip.info(
                locale_freezen.rangeRCOverErrorTitle,
                locale_freezen.rangeRCOverError
    ...
    //过滤和排序
    $("#MBLsheet-icon-autofilter").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        let itemdata = [
          {
            ...
            value: "asc",
            example:
              '<i class="iconfont MBLsheet-iconfont-shengxu" aria-hidden="true"></i>',
          },
          {
            text: locale_sort.desc,
            value: "desc",
            example:
              '<i class="iconfont MBLsheet-iconfont-jiangxu" aria-hidden="true"></i>',
          },
          {
            text: locale_sort.custom + "...",
            value: "diysort",
            example:
              '<i class="iconfont MBLsheet-iconfont-zidingyipaixu" aria-hidden="true"></i>',
          },
          { text: "", value: "split", example: "" },
          {
            ...
            value: "filter",
            example:
              '<i class="iconfont MBLsheet-iconfont-shaixuan2" aria-hidden="true"></i>',
          },
          {
            text: locale_filter.clearFilter,
            value: "clearfilter",
            example:
              '<i class="iconfont MBLsheet-iconfont-qingchushaixuan" aria-hidden="true"></i>',
          },
        ];
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          if (itemvalue == "diysort") {
            ...
            if (
              $("#MBLsheet-filter-options-sheet" + Store.currentSheetIndex)
                .length > 0
            ) {
              $("#MBLsheet-filter-initial").click();
            } else {
              createFilter();
            }
    ...
    //查找和替换
    $("#MBLsheet-icon-seachmore").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      const _locale = locale();
      ...
      if ($menuButton.length == 0) {
        let itemdata = [
          {
            ...
            value: "search",
            example:
              '<i class="iconfont MBLsheet-iconfont-sousuo" aria-hidden="true"></i>',
          },
          {
            text: locale_findAndReplace.replace + " ...",
            value: "replace",
            example:
              '<i class="iconfont MBLsheet-iconfont-tihuan" aria-hidden="true"></i>',
          },
          // {"text": locale_findAndReplace.goto+" ...", "value": "goto", "example": '<i class="iconfont MBLsheet-iconfont-zhuandao1" aria-hidden="true"></i>'},
          { text: "", value: "split", example: "" },
          {
            text: locale_findAndReplace.location + " ...",
            value: "location",
            example:
              '<i class="iconfont MBLsheet-iconfont-dingwei" aria-hidden="true"></i>',
          },
          {
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          if (itemvalue == "search" || itemvalue == "replace") {
            //查找替换
            if (itemvalue == "search") {
              MBLsheetSearchReplace.createDialog(0);
            } else if (itemvalue == "replace") {
              MBLsheetSearchReplace.createDialog(1);
            }

            MBLsheetSearchReplace.init();

            $("#MBLsheet-search-replace #searchInput input").focus();
          } else if (itemvalue == "location") {
            //定位条件
            MBLsheetLocationCell.createDialog();
            MBLsheetLocationCell.init();
          } else if (
            ...
            if (itemvalue == "locationFormula") {
              //公式
              MBLsheetLocationCell.apply(range, "locationFormula", "all");
            } else if (itemvalue == "locationConstantDate") {
              //日期
              MBLsheetLocationCell.apply(range, "locationConstant", "d");
            } else if (itemvalue == "locationConstantNumber") {
              //数字
              MBLsheetLocationCell.apply(range, "locationConstant", "n");
            } else if (itemvalue == "locationConstantString") {
              //字符
              MBLsheetLocationCell.apply(range, "locationConstant", "s,g");
            } else if (itemvalue == "locationConstantError") {
              //错误
              MBLsheetLocationCell.apply(range, "locationConstant", "e");
            } else if (itemvalue == "locationCF") {
              //条件格式
              MBLsheetLocationCell.apply(range, "locationCF");
            }
            ...
            let range = $.extend(true, [], Store.MBLsheet_select_save);

            MBLsheetLocationCell.apply(range, "locationStepRow");
            ...
            MBLsheetLocationCell.apply(range, "locationStepColumn");
          }
        });
      }
    ...
    //公式
    $("#MBLsheet-icon-function").click(function () {
      _this.autoSelectionFormula("SUM");
    });

    //公式菜单
    $("#MBLsheet-icon-function-menu").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
        ...
        $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
          ...
          if (itemvalue == "if") {
            ...
            if (cell != null && cell.f != null) {
              //单元格有计算
              let functionStr = MBLsheetformula.getfunctionParam(cell.f);
    ...
    //条件格式
    $("#MBLsheet-icon-conditionformat").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      if ($menuButton.length == 0) {
        ...
        //数据条子菜单
        let submenu3 = `<div id="MBLsheet-icon-dataBar-menuButton" class="MBLsheet-cols-menu MBLsheet-rightgclick-menu MBLsheet-menuButton MBLsheet-menuButton-sub MBLsheet-mousedown-cancel" style="width: 126px;padding: 5px;top: 118.5px;left: 1321.48px;display: none;">
                                    <div itemvalue="0" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 0;" title="${conditionformat_text.gradientDataBar_1}"></div>
                                    </div>
                                    <div itemvalue="1" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px 0;" title="${conditionformat_text.gradientDataBar_2}"></div>
                                    </div>
                                    <div itemvalue="2" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px 0;" title="${conditionformat_text.gradientDataBar_3}"></div>
                                    </div>
                                    <div itemvalue="3" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 -36px;" title="${conditionformat_text.gradientDataBar_4}"></div>
                                    </div>
                                    <div itemvalue="4" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px -36px;" title="${conditionformat_text.gradientDataBar_5}"></div>
                                    </div>
                                    <div itemvalue="5" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px -36px;" title="${conditionformat_text.gradientDataBar_6}"></div>
                                    </div>
                                    <div itemvalue="6" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 -72px;" title="${conditionformat_text.solidColorDataBar_1}"></div>
                                    </div>
                                    <div itemvalue="7" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px -72px;" title="${conditionformat_text.solidColorDataBar_2}"></div>
                                    </div>
                                    <div itemvalue="8" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px -72px;" title="${conditionformat_text.solidColorDataBar_3}"></div>
                                    </div>
                                    <div itemvalue="9" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 -108px;" title="${conditionformat_text.solidColorDataBar_4}"></div>
                                    </div>
                                    <div itemvalue="10" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px -108px;" title="${conditionformat_text.solidColorDataBar_5}"></div>
                                    </div>
                                    <div itemvalue="11" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px -108px;" title="${conditionformat_text.solidColorDataBar_6}"></div>
                                    </div>
                                </div>`;

        //色阶
        let submenu4 = `<div id="MBLsheet-icon-colorGradation-menuButton" class="MBLsheet-cols-menu MBLsheet-rightgclick-menu MBLsheet-menuButton MBLsheet-menuButton-sub MBLsheet-mousedown-cancel" style="width: 126px;padding: 5px;top: 143.5px;left: 1321.48px;display: none;">
                                    <div itemvalue="0" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 0;" title="${conditionformat_text.colorGradation_1}"></div>
                                    </div>
                                    <div itemvalue="1" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px 0;" title="${conditionformat_text.colorGradation_2}"></div>
                                    </div>
                                    <div itemvalue="2" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px 0;" title="${conditionformat_text.colorGradation_3}"></div>
                                    </div>
                                    <div itemvalue="3" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -114px 0;" title="${conditionformat_text.colorGradation_4}"></div>
                                    </div>
                                    <div itemvalue="4" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 -36px;" title="${conditionformat_text.colorGradation_5}"></div>
                                    </div>
                                    <div itemvalue="5" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px -36px;" title="${conditionformat_text.colorGradation_6}"></div>
                                    </div>
                                    <div itemvalue="6" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px -36px;" title="${conditionformat_text.colorGradation_7}"></div>
                                    </div>
                                    <div itemvalue="7" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -114px -36px;" title="${conditionformat_text.colorGradation_8}"></div>
                                    </div>
                                    <div itemvalue="8" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: 0 -72px;" title="${conditionformat_text.colorGradation_9}"></div>
                                    </div>
                                    <div itemvalue="9" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -38px -72px;" title="${conditionformat_text.colorGradation_10}"></div>
                                    </div>
                                    <div itemvalue="10" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -76px -72px;" title="${conditionformat_text.colorGradation_11}"></div>
                                    </div>
                                    <div itemvalue="11" class="MBLsheet-cols-menuitem MBLsheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
                                        <div class="MBLsheet-mousedown-cancel bgImgBox" style="background-position: -114px -72px;" title="${conditionformat_text.colorGradation_12}"></div>
                                    </div>
                                </div>`;
        ...
        $("#MBLsheet-icon-highlightCellRule-menuButton").width(160);
        $("#MBLsheet-icon-projectSelectRule-menuButton").width(180);
        ...
        $(document)
          ...
          .on(
            ...
            "#MBLsheet-icon-highlightCellRule-menuButton .MBLsheet-cols-menuitem",
            function () {
              $menuButton.hide();
              $("#MBLsheet-icon-highlightCellRule-menuButton").hide();
              MBLsheetContainerFocus();
        ...
        $(document)
          ...
          .on(
            ...
            "#MBLsheet-icon-projectSelectRule-menuButton .MBLsheet-cols-menuitem",
            function () {
              $menuButton.hide();
              $("#MBLsheet-icon-projectSelectRule-menuButton").hide();
              MBLsheetContainerFocus();
        ...
        $(document)
          ...
          .on(
            "click.CFdataBar",
            "#MBLsheet-icon-dataBar-menuButton .MBLsheet-cols-menuitem",
            function () {
              $menuButton.hide();
              $("#MBLsheet-icon-dataBar-menuButton").hide();
              MBLsheetContainerFocus();
              ...
              if (Store.MBLsheet_select_save.length > 0) {
                let cellrange = $.extend(true, [], Store.MBLsheet_select_save);
        ...
        $(document)
          ...
          .on(
            ...
            "#MBLsheet-icon-colorGradation-menuButton .MBLsheet-cols-menuitem",
            function () {
              $menuButton.hide();
              $("#MBLsheet-icon-colorGradation-menuButton").hide();
              MBLsheetContainerFocus();
        ...
        $(document)
          ...
          .on(
            "click.CFdeleteRule",
            "#MBLsheet-icon-deleteRule-menuButton .MBLsheet-cols-menuitem",
            function () {
              $menuButton.hide();
              $("#MBLsheet-icon-deleteRule-menuButton").hide();
              MBLsheetContainerFocus();
    ...
    //批注
    $("#MBLsheet-icon-postil").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      ...
      $menuButton.find(".MBLsheet-cols-menuitem").click(function () {
        ...
        if (itemvalue == "newPs") {
          MBLsheetPostil.newPs(row_index, col_index);
        } else if (itemvalue == "editPs") {
          MBLsheetPostil.editPs(row_index, col_index);
        } else if (itemvalue == "delPs") {
          MBLsheetPostil.delPs(row_index, col_index);
        } else if (itemvalue == "showHidePs") {
          MBLsheetPostil.showHidePs(row_index, col_index);
        } else if (itemvalue == "showHideAllPs") {
          MBLsheetPostil.showHideAllPs();
        }
      });
      // }
    ...
    //sheet protection
    $("#MBLsheet-icon-protection").click(function () {
      let sheetFile = sheetmanage.getSheetByIndex();
      openProtectionModal(sheetFile);
    });

    //print
    $("#MBLsheet-icon-print").click(function () {
      let menuButtonId = $(this).attr("id") + "-menuButton";
      let $menuButton = $("#" + menuButtonId);
      const _locale = locale();
        ...
        let itemdata = [
          {
            ...
            value: "print",
            example:
              '<i class="iconfont MBLsheet-iconfont-dayin" aria-hidden="true"></i>',
          },
          { text: "", value: "split", example: "" },
          {
          ...
          },
          {
            ...
            value: "rows",
            example:
              '<i class="iconfont MBLsheet-iconfont-zhuandao1" aria-hidden="true"></i>',
          },
          {
    ...
    $("body")
      .on(
        ...
        ".MBLsheet-menuButton .MBLsheet-cols-submenu",
        function (e) {
          let $t = $(this),
            attrid = $t.attr("itemvalue"),
            $attr = $("#MBLsheet-icon-" + attrid + "-menuButton");
      ...
      .on("mouseover mouseleave", ".MBLsheet-menuButton-sub", function (e) {
        if (e.type === "mouseover") {
          _this.rightclickmenu.addClass("MBLsheet-cols-menuitem-hover");
          clearTimeout(_this.submenuhide[$(this).attr("id")]);
        } else {
          _this.rightclickmenu.removeClass("MBLsheet-cols-menuitem-hover");
          $(this).hide();
        }
      });
  },
  ...
  updateFormat: function (d, attr, foucsStatus) {
    ...
    if (attr in inlineStyleAffectAttribute) {
      if (parseInt($("#MBLsheet-input-box").css("top")) > 0) {
        let value = $("#MBLsheet-input-box").text();
        if (value.substr(0, 1) != "=") {
          let cell =
            d[Store.MBLsheetCellUpdate[0]][Store.MBLsheetCellUpdate[1]];
          updateInlineStringFormat(
            cell,
            attr,
            foucsStatus,
            MBLsheetformula.rangeResizeTo
          );
          // return;
        }
      }
    ...
    for (let s = 0; s < Store.MBLsheet_select_save.length; s++) {
      let row_st = Store.MBLsheet_select_save[s]["row"][0],
        row_ed = Store.MBLsheet_select_save[s]["row"][1];
      let col_st = Store.MBLsheet_select_save[s]["column"][0],
        col_ed = Store.MBLsheet_select_save[s]["column"][1];
    ...
    jfrefreshgrid(d, Store.MBLsheet_select_save, allParam, false);
  },
  updateFormat_mc: function (d, foucsStatus) {
    ...
    if (foucsStatus == "mergeCancel") {
      for (let i = 0; i < Store.MBLsheet_select_save.length; i++) {
        let range = Store.MBLsheet_select_save[i];
        let r1 = range["row"][0],
          r2 = range["row"][1];
    ...
    jfrefreshgrid(d, Store.MBLsheet_select_save, { cfg: cfg });
  ...
  changeMenuButtonDom: function (attr, foucsStatus, _locale) {
    ...
    if (attr == "bl") {
      if (foucsStatus != "0") {
        $("#MBLsheet-icon-bold").addClass("MBLsheet-toolbar-button-hover");
      } else {
        $("#MBLsheet-icon-bold").removeClass("MBLsheet-toolbar-button-hover");
      }
    } else if (attr == "it") {
      if (foucsStatus != "0") {
        $("#MBLsheet-icon-italic").addClass("MBLsheet-toolbar-button-hover");
      } else {
        $("#MBLsheet-icon-italic").removeClass("MBLsheet-toolbar-button-hover");
      }
    } else if (attr == "cl") {
      if (foucsStatus != "0") {
        $("#MBLsheet-icon-strikethrough").addClass(
          "MBLsheet-toolbar-button-hover"
        );
      } else {
        $("#MBLsheet-icon-strikethrough").removeClass(
          "MBLsheet-toolbar-button-hover"
        );
      }
    } else if (attr == "un") {
      if (foucsStatus != "0") {
        $("#MBLsheet-icon-underline").addClass("MBLsheet-toolbar-button-hover");
      } else {
        $("#MBLsheet-icon-underline").removeClass(
          "MBLsheet-toolbar-button-hover"
        );
      }
    } else if (attr == "ff") {
      let menuButtonId = "MBLsheet-icon-font-family-menuButton";
      let $menuButton = $("#" + menuButtonId);
      // const locale_fontarray = locale().fontarray;
      ...
      let $menuButton = $("#MBLsheet-icon-align-menu-menuButton");
      let $t = $("MBLsheet-icon-align"),
        itemvalue = "left";
      ...
      let $menuButton = $("#MBLsheet-icon-valign-menu-menuButton");
      let $t = $("MBLsheet-icon-valign"),
        itemvalue = "bottom";
      ...
      let $menuButton = $("#MBLsheet-icon-textwrap-menu-menuButton");
      let $t = $("MBLsheet-icon-textwrap"),
        itemvalue = "clip";
      ...
      let $menuButton = $("#MBLsheet-icon-rotation-menu-menuButton");
      let $t = $("MBLsheet-icon-rotation"),
        itemvalue = "none";
      ...
      let $menuButton = $("#MBLsheet-icon-fmt-other");
      const _locale = locale();
      const locale_defaultFmt = _locale.defaultFmt;
      if (!foucsStatus) {
        $menuButton
  ...
  getCellRealSize: function (d, cell_r, cell_c) {
    ...
    if (!!celldata && celldata["mc"] != null) {
      ...
      let config =
        get_MBLsheetfile()[getSheetIndex(Store.currentSheetIndex)]["config"];
  ...
  getTextSize: function (text, font) {
    ...
    if ($("#MBLsheetTextSizeTest").length == 0) {
      $(
        '<span id="MBLsheetTextSizeTest" style="float:left;white-space:nowrap;visibility:hidden;margin:0;padding:0;">' +
          text +
          "</span>"
      ).appendTo($("body"));
    }

    let o = $("#MBLsheetTextSizeTest").text(text).css({ font: f }),
      w = o.innerWidth(),
      h = o.innerHeight();
  ...
  activeFormulaInput: function (
    ...
    if (isnull) {
      ...
      let $span = $("#MBLsheet-rich-text-editor").find("span");
      MBLsheetformula.setCaretPosition($span.get($span.length - 2), 0, 1);
    ...
    let formulaTxt =
      '<span dir="auto" class="MBLsheet-formula-text-color">=</span><span dir="auto" class="MBLsheet-formula-text-color">' +
      formula.toUpperCase() +
      '</span><span dir="auto" class="MBLsheet-formula-text-color">(</span><span class="MBLsheet-formula-functionrange-cell" rangeindex="0" dir="auto" style="color:' +
      luckyColor[0] +
      ';">' +
      getRangetxt(
      ...
      '</span><span dir="auto" class="MBLsheet-formula-text-color">)</span>';
    $("#MBLsheet-rich-text-editor").html(formulaTxt);

    MBLsheetformula.israngeseleciton();
    MBLsheetformula.rangestart = true;
    MBLsheetformula.rangedrag_column_start = false;
    MBLsheetformula.rangedrag_row_start = false;
    MBLsheetformula.rangechangeindex = 0;
    MBLsheetformula.rangeSetValue({ row: rowh, column: columnh });
    MBLsheetformula.func_selectedrange = {
      left: col_pre,
      width: col - col_pre - 1,
      top: row_pre,
      height: row - row_pre - 1,
  ...
  backFormulaInput: function (d, r, c, rowh, columnh, formula) {
    ...
    let f =
      ...
      ")";
    let v = MBLsheetformula.execfunction(f, r, c);
    let value = { v: v[1], f: v[2] };
    setcellvalue(r, c, d, value);
    MBLsheetformula.execFunctionExist.push({
      r: r,
      c: c,
  ...
  autoSelectionFormula: function (formula) {
    ...
    let nullfindnum = 40;
    let isfalse = true;
    MBLsheetformula.execFunctionExist = [];
    ...
    for (let s = 0; s < Store.MBLsheet_select_save.length; s++) {
      let st_r = Store.MBLsheet_select_save[s].row[0],
        ed_r = Store.MBLsheet_select_save[s].row[1];
      let st_c = Store.MBLsheet_select_save[s].column[0],
        ed_c = Store.MBLsheet_select_save[s].column[1];
      let row_index = Store.MBLsheet_select_save[s].row_focus,
        col_index = Store.MBLsheet_select_save[s].column_focus;
    ...
    if (!isfalse) {
      MBLsheetformula.execFunctionExist.reverse();
      MBLsheetformula.execFunctionGroup(null, null, null, null, d);
  ...
  addFontTolist: function (fontName) {
    ...
    if (isNone) {
      ...
      ret.text =
        "<span class='MBLsheet-mousedown-cancel' style='font-size:11px;font-family:" +
        fontName +
        "'>" +
        fontName +
        "</span>";
      ...
      let $menuButton = $("#MBLsheet-icon-font-family-menuButton");
