...
function renderCharts(chartLists, isDemo) {
  ...
  for (let i = 0; i < chartLists.length; i++) {
    ...
    let modelChartShowHTML =
      '<div id="${id}"class="MBLsheet-modal-dialog MBLsheet-modal-dialog-chart ${addclass}"tabindex="0"role="dialog"aria-labelledby=":41e"dir="ltr"><div class="MBLsheet-modal-dialog-resize"><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-lt"data-type="lt"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-mt"data-type="mt"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-lm"data-type="lm"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-rm"data-type="rm"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-rt"data-type="rt"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-lb"data-type="lb"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-mb"data-type="mb"></div><div class="MBLsheet-modal-dialog-resize-item MBLsheet-modal-dialog-resize-item-rb"data-type="rb"></div></div><div class="MBLsheet-modal-dialog-controll"><span class="MBLsheet-modal-controll-btn MBLsheet-modal-controll-update"role="button"tabindex="0"aria-label="修改图表"title="修改图表"><i class="fa fa-pencil"aria-hidden="true"></i></span><span class="MBLsheet-modal-controll-btn MBLsheet-modal-controll-max"role="butMBLsheet_chartIns_indexton"tabindex="0"aria-label="最大化"title="最大化"><i class="fa fa-window-maximize"aria-hidden="true"></i></span><span class="MBLsheet-modal-controll-btn MBLsheet-modal-controll-del"role="button"tabindex="0"aria-label="删除"title="删除"><i class="fa fa-trash"aria-hidden="true"></i></span></div><div class="MBLsheet-modal-dialog-content">${content}</div></div>';

    let $t = $(
      replaceHtml(modelChartShowHTML, {
        id: chart_id_c,
        addclass: "MBLsheet-data-visualization-chart",
        title: "图表生成",
        content: "",
      })
    ).appendTo($(".MBLsheet-cell-main"));

    setChartMoveableEffect($t);

    $(`#${chart_id_c}`).children(".MBLsheet-modal-dialog-content")[0].id =
      chart_id;
    ...
    $(`#${chart_id}_c .MBLsheet-modal-controll-del`).click(function (e) {
      delChart(chart_id);
    });

    // edit current chart
    $(`#${chart_id}_c .MBLsheet-modal-controll-update`).click(function (e) {
      showChartSettingComponent();
    });

    $t.children(".MBLsheet-modal-dialog-content").mousedown(function (e) {
      if (!chartInfo.chartparam.MBLsheetCurrentChartMaxState) {
        //当前图表显示区域高亮
        showNeedRangeShow(chart_id);
      }
      e.stopPropagation();
    ...
    $t.mousedown(function (e) {
      ...
      if (!chartInfo.chartparam.MBLsheetCurrentChartMaxState) {
        ...
        //允许拖动渲染框
        if (
          !$(e.target).is(".MBLsheet-modal-dialog-controll") &&
          !$(e.target).is(".MBLsheet-modal-controll-btn") &&
          !$(e.target).is("i")
        ) {
          // Debounce
          chartInfo.chartparam.MBLsheetCurrentChartMoveTimeout = setTimeout(
            function () {
              chartInfo.chartparam.MBLsheetCurrentChartMove = true;
            },
            100
          );
        }

        var toffset = chartInfo.chartparam.MBLsheetCurrentChartMoveObj.offset();
        var tpsition =
          chartInfo.chartparam.MBLsheetCurrentChartMoveObj.position();
        //MBLsheetCurrentChartMoveXy: [鼠标点相对chart框的距离X方向，鼠标点相对chart框的距离Y方向，chart框相对cell-main的距离X方向，chart框相对cell-main的距离Y方向，水平滚动条的位置，垂直滚动条的位置]
        chartInfo.chartparam.MBLsheetCurrentChartMoveXy = [
        ...
        chartInfo.chartparam.MBLsheetCurrentChartMoveWinH = $(
          "#MBLsheet-cell-main"
        )[0].scrollHeight;
        chartInfo.chartparam.MBLsheetCurrentChartMoveWinW = $(
          "#MBLsheet-cell-main"
        )[0].scrollWidth;

        if (
          !$(e.target).hasClass("MBLsheet-mousedown-cancel") &&
          ...
          $("#MBLsheet-rightclick-menu").hide();
          $(
            "#MBLsheet-sheet-list, #MBLsheet-rightclick-sheet-menu, #MBLsheet-user-menu"
          ).hide();
          $(
            "body > .MBLsheet-filter-menu, body > .MBLsheet-filter-submenu, body > .MBLsheet-cols-menu"
          ).hide();
        }
      ...
      .find(".MBLsheet-modal-dialog-resize-item")
      .mousedown(function (e) {
        if (chartInfo.chartparam.MBLsheetCurrentChartActive) {
          chartInfo.chartparam.MBLsheetCurrentChartResize =
          ...
          var position =
            chartInfo.chartparam.MBLsheetCurrentChartResizeObj.position();
          ...
          chartInfo.chartparam.MBLsheetCurrentChartResizeXy = [
            x,
            y,
            $t.width(),
            $t.height(),
          ...
          chartInfo.chartparam.MBLsheetCurrentChartResizeWinH = $(
            "#MBLsheet-cell-main"
          )[0].scrollHeight;
          chartInfo.chartparam.MBLsheetCurrentChartResizeWinW = $(
            "#MBLsheet-cell-main"
          )[0].scrollWidth;

          chartInfo.chartparam.MBLsheetCurrentChart = chart_id;
...
function jfrefreshchartall(flowdata1, r_st, r_ed, c_st, c_ed) {
  ...
  if (chart.rangeArray.length == 1) {
    ...
    //根据原有的范围取得数据
    var MBLsheetgetcellrange = formula.getcellrange(chart.rangeTxt);
    var sheetIndex =
      MBLsheetgetcellrange.sheetIndex == -1
        ? 0
        : MBLsheetgetcellrange.sheetIndex; //sheetIndex为-1时，转化为0

    var selection = {
      row: MBLsheetgetcellrange.row,
      column: MBLsheetgetcellrange.column,
      dataSheetIndex: sheetIndex,
    }; //数组
    var getcelldata = MBLsheet_getcelldata(chart.rangeTxt);
...
function chart_selection() {
  return {
    create: function () {
      ...
      $("#MBLsheet-chart-rangeShow").empty();
      ...
      $("#MBLsheet-chart-rangeShow").append(
      ...
      function getRangeShowHtml(type, r1, r2, c1, c2) {
        ...
        var html =
          '<div id="MBLsheet-chart-rangeShow-' +
          type +
          '" style="left: ' +
          col_pre +
          "px;width: " +
          ...
          '<div class="MBLsheet-chart-rangeShow-move" data-type="top" style="height: 2px;border-top: 2px solid #fff;border-bottom: 2px solid #fff;background: ' +
          color +
          ';position: absolute;left: 0;right: 0;top: -2px;z-index: 18;opacity: 0.9;cursor: move;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-move" data-type="right" style="width: 2px;border-left: 2px solid #fff;border-right: 2px solid #fff;background: ' +
          color +
          ';position: absolute;top: 0;bottom: 0;right: -2px;z-index: 18;opacity: 0.9;cursor: move;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-move" data-type="bottom" style="height: 2px;border-top: 2px solid #fff;border-bottom: 2px solid #fff;background: ' +
          color +
          ';position: absolute;left: 0;right: 0;bottom: -2px;z-index: 18;opacity: 0.9;cursor: move;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-move" data-type="left" style="width: 2px;border-left: 2px solid #fff;border-right: 2px solid #fff;background: ' +
          color +
          ...
          '<div class="MBLsheet-chart-rangeShow-resize" data-type="lt" style="width: 6px;height: 6px;border: 1px solid #fff;background: ' +
          color +
          ';position: absolute;left: -3px;top: -3px;z-index: 19;cursor: se-resize;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-resize" data-type="rt" style="width: 6px;height: 6px;border: 1px solid #fff;background: ' +
          color +
          ';position: absolute;right: -3px;top: -3px;z-index: 19;cursor: ne-resize;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-resize" data-type="lb" style="width: 6px;height: 6px;border: 1px solid #fff;background: ' +
          color +
          ';position: absolute;left: -3px;bottom: -3px;z-index: 19;cursor: ne-resize;"></div>' +
          '<div class="MBLsheet-chart-rangeShow-resize" data-type="rb" style="width: 6px;height: 6px;border: 1px solid #fff;background: ' +
          color +
    ...
    rangeMoveDraging: function (event, sheetBarHeight, statisticBarHeight) {
      ...
      var scrollLeft = $("#MBLsheet-cell-main").scrollLeft();
      var scrollTop = $("#MBLsheet-cell-main").scrollTop();
...
function createLuckyChart(width, height, left, top) {
  //如果只选中一个单元格，则自动填充选取
  var jfgird_select_save = MBLsheet.getMBLsheet_select_save();
  if (
    ...
    jfgird_select_save = MBLsheet.getMBLsheet_select_save();
  }
  ...
  for (
    ...
    for (
      ...
      var value = getcellvalue(r, c, MBLsheet.flowdata());
  ...
  MBLsheet.setMBLsheet_select_save(jfgird_select_save);

  chartInfo.MBLsheet_shiftpositon = $.extend(true, {}, jfgird_select_save[0]);
  MBLsheetMoveEndCell("down", "range", false, row_ed);
  jfgird_select_save = MBLsheet.getMBLsheet_select_save();
  ...
  chartInfo.MBLsheet_shiftpositon = $.extend(true, {}, jfgird_select_save[0]);
  MBLsheetMoveEndCell("right", "range", false, column_ed);
  jfgird_select_save = MBLsheet.getMBLsheet_select_save();
  ...
  let { render, chart_json } = chartInfo.createChart(
    $(`#${chart_id_c}`).children(".MBLsheet-modal-dialog-content")[0],
    chartData,
    chart_id,
    rangeArray,
    rangeTxt
  ...
  // insert chartinfo
  let sheetFile =
    chartInfo.MBLsheetfile[getSheetIndex(chartInfo.currentSheetIndex)];
  ...
  $t.mousedown(function (e) {
    ...
    .mousedown(function (e) {
      if (chartInfo.chartparam.MBLsheetCurrentChartActive) {
        chartInfo.chartparam.MBLsheetCurrentChartResize = $(this).data("type"); //开始状态resize
...
function setChartMoveableEffect($container) {
  $container.find(".MBLsheet-modal-dialog-content").hover(
    function () {
...
function delChart(chart_id) {
  // delete container
  $(`.MBLsheet-cell-main #${chart_id}_c`).remove();
...
function showNeedRangeShow(chart_id) {
  let chartLists =
    chartInfo.MBLsheetfile[getSheetIndex(chartInfo.currentSheetIndex)].chart;
...
function selectRangeBorderShow(chart_id) {
  ...
  chartInfo.chartparam.MBLsheetCurrentChartActive = true;
  chartInfo.chartparam.MBLsheetCurrentChartMoveObj = $t;
  chartInfo.chartparam.MBLsheetCurrentChartResizeObj = $t;
  chartInfo.chartparam.MBLsheetCurrentChart = chart_id;

  //MBLsheet取cell-main，后续扩展到其他的用户自定义元素
  $("#MBLsheet-cell-main")
    .find(".MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-resize")
    .hide();
  $("#MBLsheet-cell-main")
    .find(".MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-controll")
    .hide();

  $t.css("z-index", chartInfo.chartparam.MBLsheetCurrentChartZIndexRank++);
  $t.find(".MBLsheet-modal-dialog-resize").show();
  $t.find(".MBLsheet-modal-dialog-controll").show();

  if (
    ($(".chartSetting").is(":visible") ||
      chartInfo.chartparam.MBLsheet_chart_redo_click) &&
    chart_id != chartInfo.chartparam.MBLsheetCurrentChart
  ) {
...
function selectRangeBorderHide(settingShow) {
  $(
    "#MBLsheet-cell-main .MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-resize, #MBLsheet-cell-main .MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-controll"
  ).hide();
  $("#MBLsheet-cell-main")
    .find(".MBLsheet-datavisual-selection-set div")
    .remove();
  chartInfo.chartparam.MBLsheetCurrentChartActive = false;
...
function hideChartSettingComponent(refresh) {
  if ($(".chartSetting").is(":visible")) {
    //隐藏设置界面
    $(".chartSetting").hide();
    //.MBLsheet-modal-dialog-resize为图表显示框的缩放框，.MBLsheet-modal-dialog-controll为显示框右边的控制按钮
    $(
      "#MBLsheet-cell-main .MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-resize, #MBLsheet-cell-main .MBLsheet-modal-dialog-chart .MBLsheet-modal-dialog-controll"
    ).hide();
...
function renderChartShow(index) {
  ...
  const MBLsheetfile = chartInfo.MBLsheetfile;
  MBLsheetfile.forEach((file) => {
    //切换当前页的所有图表都显示出来
    if (file.index == index) {
